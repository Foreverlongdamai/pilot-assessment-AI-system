...
| M7 WinUI Expert Designer | M7A 后端已工程验证；M7B Task 1–10 已完成正式 WinUI scaffold、强类型合同层、受监督 sidecar、受管 project/session、canonical task-scheme、global active/dim 模型图、图上编辑、多独立节点窗口，以及 schema/operator 驱动的 Raw Input 与 Evidence 完整编辑器；Task 11 起继续实现 BN state/parent/CPT 编辑器与后续 autosave、双语和运行结果工作区 |
...
开发环境可用 `python -m pilot_assessment.sidecar` 启动 M6 本地 stdio 后端。它不监听网络端口；第一个请求必须是 `runtime.hello`。这是供 M7 Windows 前端调用的后端入口，不是最终安装包。
