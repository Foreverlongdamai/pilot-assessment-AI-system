...
| M7 WinUI Expert Designer | M7A 后端已工程验证；M7B Task 1–9 已完成正式 WinUI scaffold、强类型合同层、受监督 sidecar、受管 project/session、canonical task-scheme、global active/dim 模型图、图上编辑，以及可并行移动、缩放和恢复位置的独立节点窗口；Task 10 起继续实现 Raw Input、Evidence、BN/CPT 完整编辑器 |
