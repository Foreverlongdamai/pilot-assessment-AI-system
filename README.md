...
当前状态：**产品已于 2026-07-15 重基线为面向领域专家的可视化评估模型设计系统。M1/M2/M3 已工程验证；旧 M4 Task 0–28 已完成，现有 O1–O12/H1–H3 whole-Anchor plugins、共享 primitives 与三个 preprocessing providers 均保留为迁移和历史重放来源。旧 Task 29–36 已暂停，不再继续 H4/H5/O13 固定插件路线。当前权威方向采用 canonical `EvidenceRecipe`、typed operator graph、自动生成的参数表单，以及“自动保存草稿 + 一键应用到后续评估”；普通新增/修改 Anchor、计算方法、BN 节点/边/CPT 不需要 Python 发布、审批或逐次工程测试。真实状态是 M4R 架构已获确认、合并后的正式规格待用户最终复核、实现尚未开始；完整产品仍为 `in_progress`，`formal_run_authorized=false`。当前 18 个 Anchor 和 33-node BN 仅是 starter templates，不构成科学有效性声明。**
...
这些结果只验证相应的软件合同和运行路径。格式样例是一次随意飞行记录，只用于格式、接口和时间路径；结果不证明轨迹、任务、表现、生理指标、Anchor 或飞行员能力有效。旧 M4 Task 0–28 的测试数字继续作为当时代码的历史工程证据，但不再充当新 M4R 的完成门，也不要求专家后续修改维持这些 provisional 算法的输出等价。现有十五个 whole-Anchor capabilities 与三个 preprocessing providers 均保留；M4R 尚未实现，所有路径保持 `formal_run_authorized=false`。
