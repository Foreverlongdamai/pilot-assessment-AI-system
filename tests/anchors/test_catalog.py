...
def test_registry_resource_honestly_declares_o1_o2_o3() -> None:
    ...
    assert [entry["anchor_id"] for entry in document["entries"]] == ["O1", "O2", "O3"]
    assert document["preprocessors"] == []
    registry = AnchorRuntimeRegistry.model_validate_json(raw)
    assert tuple(entry.anchor_id for entry in registry.entries) == ("O1", "O2", "O3")
