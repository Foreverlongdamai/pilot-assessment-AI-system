# User acceptance checklist — v0.1.0-rc.4
...
- [ ] Holding the primary button and moving a normal node at least 4 px visibly drags it; an ordinary
  click still selects it, and releasing the pointer leaves the node at the new position.
- [ ] Each green Raw Input Family root can be dragged and remains a display-only layout projection.
- [ ] **Save All** and `Ctrl+S` flush open editors and pending layout, persist both normal-node and
  Raw Input Family positions, keep the software open, and restore those positions after restart.
...
- [ ] The toolbar contains only new, details, activate, deactivate, delete, copy and paste; hovering
  each button shows its actual action in the selected UI language and never an unrelated `Ctrl+C`.
...
- [ ] RC.1, RC.2 and RC.3 are recorded as `changes-required`; RC.4 remains `pending` until this exact
