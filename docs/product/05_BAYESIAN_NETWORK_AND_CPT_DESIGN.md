...
- 状态：可编辑 starter BN 设计，尚未实现或科学验证；2026-07-15 已纳入 expert-editable rebaseline
...
> **当前权威补充：** BN Graph 与 Evidence Computation Graph 属于同一个 autosaved model draft；BN evidence node 通过稳定 binding 指向 `EvidenceRecipe` output。专家修改节点、边、state、CPT 或 binding 后可直接 apply 到后续评估，只受 DAG、引用、shape 和概率等技术校验，不受科学审批或 golden gate。详见 [Expert-Editable Evidence and Assessment Model Design](./specs/2026-07-15-expert-editable-evidence-and-model-design.md)。
...
以上映射构成 v0.1 starter 的默认语义结构。图编辑器可以在 draft 中修改结构；每个 applied revision 都保存自己的完整节点、边和 CPT。
...
若专家需要 phase plate，可在 draft 中显式 unroll 为具有稳定 instance ID 的节点（例如 O1@translation），定义 shared parents、跨 phase 依赖和 fusion/CPT 规则。Apply 自动创建新的 expert-model revision identity；不要求手工发布 major profile。它不是 starter snapshot 的默认推理方式。
...
每个 applied model revision 至少保存：

- model_id；
- schema_version；
- revision_id；
- parent_revision_id；
- apply 前 draft_id 与最终 graph_version；
- EvidenceRecipe、operator requirements、graph、CPT 与 bindings 的 content identity；
- revision_lifecycle：applied、archived 或 superseded；
- software_verification_status、scientific_validation_status 与 permitted_use；
- created_by、created_at、可选 note；
- canonical model hash。

每次正式 assessment inference 保存：

- session ID 和输入 hash；
- applied revision_id；executable draft 可以 preview，但不能覆盖 applied revision 的历史结果；
- recipe/operator/graph/CPT/binding identities；
...
Applied revision 不可原地修改。任何修改都从已应用 revision 创建或恢复 autosaved draft；apply 后形成新的 immutable revision。
