...
Translation、Deceleration 和 Hover 是已知任务上下文，不是能力节点，也不是第四种 evidence 状态。v0.1 中它们只用于 AnchorPlugin 内部计算 phase breakdown、质量和聚合规则。

默认 BN 严格保持 33 个语义节点，并使用：

P(E_session | parent sub-skills)

每个 anchor 每个 session 只向对应 evidence node 提交一次聚合 observation。AnchorResult 保存逐 phase 数值、状态和质量，再按该 anchor 的 aggregation policy 形成单一 D/A/U 或 virtual evidence。不得把 T/D/H 三条结果重复注入同一个静态 evidence node，否则会覆盖观测或重复计算证据。v0.1 inference 只使用 base/session-aggregate CPT，不选择 translation/deceleration/hover CPT slice。
...
O8 使用 O1 和 O5 的信息，O13 使用控制与生理耦合信息。若把它们和来源 anchor 当成完全条件独立证据，可能导致后验过度集中。
...
默认 O8 和 O13 的 likelihood_strength s 均为 0.50。先得到质量处理后的 L_quality，再向无信息分布 U=(1/3,1/3,1/3) 收缩：
...
BN 外 metadata 直接保留 AnchorResult 的 calculation_status：computed、invalid_quality、missing_input、not_applicable、not_computable、dependency_missing 或 extractor_error，不再创造第二套近义状态。另设 observation_mode：hard、virtual 或 omitted。

| AnchorResult / 操作 | observation_mode | 说明 |
|---|---|---|
| computed，likelihood 为 one-hot 且质量充分 | hard | 注入明确 D/A/U |
| computed，soft likelihood 或质量低于 1 但仍通过 quality gate | virtual | 按 q 向均匀 likelihood 收缩 |
| invalid_quality、missing_input、not_applicable、not_computable、dependency_missing、extractor_error | omitted | 不向 BN 注入观测 |
| 用户显式排除 | omitted | calculation_status 不改写；另存 excluded_by_user、reason、actor 与 audit ID |

export_pending 保存在 AnchorResult.input_status_snapshot 中，并映射为 missing_input + source_export_pending reason。推理引擎对 omitted 节点自动边缘化；未观测的叶 evidence 不应改变 competency posterior。
...
质量系数 q ∈ [0,1] 用于把 soft likelihood 与无信息 likelihood 混合：

L_quality = q × L_raw + (1 - q) × (1/3, 1/3, 1/3)

q=0 时等价于不提供有方向性的证据；q=1 时完整使用 anchor scorer 的结果。

reference-v0.1 使用 binary_quality_v1，因此通过 hard gates 的 computed anchor 默认 q=1，未通过则 omitted。只有后续 model revision 显式配置并验证 graded quality scorer 时才产生 0<q<1。
...
7. 计算 evidence coverage 和 explainability；
...
Coverage 与 posterior 分开计算。每个 evidence-to-sub-skill 关系具有非负 coverage_weight，并在相关 sub-skill 内归一化。

单条 evidence 的有效覆盖贡献为：

effective_i =
available_i
× (1 - H(L_model,i) / log(3))

其中 L_model,i 是真正注入 BN 的最终 likelihood：先经过 scorer、quality mixing，再经过 derived-evidence strength mixing。不得再乘一次 quality，避免双重折损。available_i=1 仅当 observation_mode 为 hard/virtual；missing、invalid、dependency_missing、extractor_error 或 user-excluded 为 0。完全均匀 likelihood 的信息量为 0；清晰 one-hot evidence 的信息量为 1。

聚合 coverage：

coverage =
Σ(weight_i × effective_i)
/
...
- raw availability coverage；
- effective information coverage。

默认评估状态：

| Effective coverage | 状态 |
|---:|---|
| ≥ 0.70 | assessable |
| ≥ 0.35 且 < 0.70 | partial |
...
not_applicable evidence 不进入分母；缺失但本应存在的 evidence 进入分母且贡献为 0。若某个聚合视图的分母为 0，coverage 值为 null、状态为 not_applicable，不能除零或误报 prior_only。prior_only 只用于分母>0、没有任何方向性 observation 的 competency/sub-skill。
