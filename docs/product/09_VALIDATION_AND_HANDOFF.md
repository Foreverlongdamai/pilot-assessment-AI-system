...
|---|---|
| 设计版本 | v0.1 |
| 当前软件状态 | in_progress（M1/M2/M3 已 engineering verified；M4 replacement Task 0–4 已分别由 `bc544bf`、`f56365c`、`928e9a4`、`e054620`、`1528d09` 完成，catalog/plan/request contracts 已实现，下一步为 Task 5 measurement/artifact/inventory/report contracts；18/18 已设计、0/18 production plugins 已实现；M4 与完整 Core alpha 尚未 engineering verified，Gate B 尚未完成） |
...
- 后端 M1/M2/M3 已有合同、directory-bundle loader、JSON Schema、版本化 adapters/bindings、deterministic multimodal software fixtures、`IngestionReadinessReport`、native-rate `AlignedSession`/`SynchronizationReport` 和自动化测试。M4 已完成 18/18 anchor 的书面设计以及 replacement Task 0–4；`AnchorResultV2`、semantic/reference binding、catalog/dependency/execution-plan 与 immutable-request contracts 已进入生产代码。轻量验证修订、Task 3 candidate-binding 修订、D-026–D-028 与 replacement plan 已接受，原实施计划已被取代。下一步为 Task 5 measurement/artifact/inventory/report contracts，仍是 0/18 production plugins；evidence scorer、BN、assessment runner、受管理 importer、sidecar 与 WinUI 也仍未实现。因此 M1/M2/M3 已 engineering verified，而 M4 与完整产品 software verification 仍为 in_progress，Gate B 尚未通过。
