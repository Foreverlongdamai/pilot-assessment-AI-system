...
### AR-030 — M4 Task 28 H3 Off-task Dwell (INLINE)

| Field | Record |
|---|---|
| Artifacts | `anchors/plugins/h3_off_task_dwell.py`, shared `gaze-aoi-intervals-v1` consumption, exact `phase-off-task-dwell-v0.1` publication, focused shared-product/pooling/boundary/status tests, packaged H3 registry entry and registry/catalog/package honesty updates |
| Approval | Covered by the approved replacement Task 28 and the user's explicit INLINE continuation instruction; no accepted H3 formula, 5/15-percent threshold, public schema, CPT, formal-run boundary or scientific-validity claim changed |
| Finalization mode | **Implemented and self-reviewed INLINE** to conserve quota. No subagent was created and no independent/external verdict is claimed |
| TDD evidence | RED: expected collection failure while H3 was absent; three registry-honesty failures then proved the stale 14-plugin boundary, and the controlled gate exposed the final stale catalog/package inventory assertions before explicit updates. GREEN: focused H3 `12 passed`; controlled gaze/H1/fixation/H2/H3/preprocessing/registry/catalog/DAG/service/scoring/artifact/temporal/package M4-E gate `244 passed`. Registry verify, Ruff check/format, `ty check src` and `git diff --check` passed. The full repository/build/isolated-wheel gates were intentionally not repeated; Task 20 remains the latest full stage-gate evidence (`1275 passed, 3 skipped`) |
| Inline self-review decisions saved for later audit | H3 never recomputes raw scene/gaze allocation. H1 and H3 bind the same session-scope `gaze-aoi-intervals-v1` recipe; an exact resolver test proves one provider call, object identity and logical hash identity. Every AOI with semantic `off_task=true`, including invalid-association `other_scene`, contributes to the numerator while remaining in the common denominator. Phase durations are pooled before division. Complete off-task gaze is computed U, and present I/G with no nonzero gaze dwell is `computed + U + no_gaze_dwell`, never a synthetic zero-percent Desired result. M5 alone will apply the shared `gaze_allocation` dependence group |
...
| Current status | M1–M3 engineering verified; legacy M4 Task 0–28 complete; 15 legacy/reference plugins and three providers preserved; M4R written specification approved and implementation started; `formal_run_authorized=false`. |
