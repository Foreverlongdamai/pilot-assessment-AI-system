...
10. 用户为正式运行选择一个 published revision。后端锁定 revision，解析 model-bundle reference，并由 Run Preflight 计算各 competency 的证据覆盖和可评估状态；只有 `RunPreflightReport` 可以决定是否创建 AssessmentRun。
11. 用户启动评估。sidecar 按该 published revision 建立 run snapshot，依次执行 anchor、evidence 和 BN inference；已经建立的同步结果按 fingerprint 绑定到该 snapshot。
12. Windows 前端显示运行进度，并允许取消。
13. 结果页显示四项 competency posterior、可评估状态、证据覆盖、anchor trace、缺失证据和限制说明。
14. 只有达到该 competency 的覆盖质量门时，系统才生成 weak-skill diagnosis。
