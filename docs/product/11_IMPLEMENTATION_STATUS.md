# Implementation Status — M1/M2/M3/M4R/M5/M6/M7A Engineering Verified; M7B Task 1–8 Complete
...
| 下一里程碑 | 从 M7B Task 9 开始实现可同时打开、移动、缩放并恢复位置的独立节点窗口，再进入 Raw/Evidence/BN 完整编辑器；M8 packaging 仍放在最后阶段 |
| 软件状态 | `in_progress`（M1/M2/M3/M4R/M5/M6/M7A engineering verified；M7B Task 1–8 已完成，M7B 多浮窗、完整专家编辑、运行结果与 M8 packaging 尚未完成，starter/synthetic `formal_run_authorized=false`） |
...
M7A 已按 12-task INLINE plan 完成，current-model/automatic-snapshot 后端门已关闭。M7B Task 1–8 现已完成 WinUI 工程、强类型合同、受监督 sidecar、真实应用 shell、受管 project/session 工作区、canonical task-scheme navigation、backend-driven global active/dim graph，以及节点新建、激活/级联停用、复制/粘贴、拖拽布局与 typed edge 编辑；下一步从 Task 9 开始实现可并行打开的独立节点窗口。不能把它重新编号为 M6、回到旧 fixed-plugin Task 29，或提前做 M8 installer。M7B 必须消费 M7A current DTO，而不是把旧 draft/publish DTO 重新带回正常 UI：
...
- [M7 WinUI Expert Designer and Task Activation Workspace Design](specs/2026-07-17-m7-winui-expert-designer-and-task-activation-workspace-design.md)（当前产品语义；M7A 后端已实施，M7B Task 1–8 已完成）
...
- [M7B WinUI Expert Designer Implementation Plan](plans/2026-07-17-m7b-winui-expert-designer-implementation-plan.md)（当前 15 个 INLINE 前端任务；Tasks 1–8 已完成，Task 9 next）
