# Implementation Status — RC.3 Engineering Verified, User Acceptance Pending

| 字段 | 当前值 |
|---|---|
| 状态日期 | 2026-07-21 |
| 产品设计基线 | v0.1.0-rc.3 portable expert-designer release candidate（D-031–D-087） |
...
| 下一里程碑 | 用户独立验收 annotated `v0.1.0-rc.3`；不移动 RC.1/RC.2/RC.3 标签 |
| 软件状态 | `engineering_verified / user_acceptance=pending`（M1–M8E engineering history preserved；RC.1/RC.2 user acceptance=`changes-required`；RC.3 tagged candidate gate 已关闭；starter/synthetic `formal_run_authorized=false`） |
...
M1/M2/M3、M4R、M5、M6、M7、M8A、M8B、M8C-1、M8D、D-055 与 M8E 的 engineering history 保持完整。RC.1 因根目录 94 folders / 374 files 被标记为 `changes-required`；clean annotated RC.2 已在仓库外证明 8 directories / 2 files / 1 launcher，但用户继续发现 Assessment、图标、删除和拖动四项缺陷，因此 RC.2 同样为 `changes-required`。D-084–D-087 的 RC.3 修订、文档、annotated tag、ZIP 和仓库外 restricted-PATH 验证均已完成，当前交由用户独立验收。系统当前工程实现可以：
...
本返修只存在于 RC.3 tag 之后的 `main` 与 disposable preview；`v0.1.0-rc.3` tag、ZIP、checksum 和既有文档保持不可变。它没有生成或冒充新的正式候选；完成当前用户检查后，下一次打包必须使用新的 candidate sequence，并重新生成受影响的双语手册与发布证据。详细边界见 [Active Save All Verification](reviews/2026-07-22-active-save-all-verification.md)。
...
本返修与 D-088 一样只存在于 RC.3 之后的 `main`；不可改写 RC.3 历史候选。详细证据见 [Node Drag Persistence Verification](reviews/2026-07-22-node-drag-persistence-verification.md)。
...
- 用户对 RC.3 的独立验收；RC.1/RC.2 均已记录为 `changes-required`，RC.3 保持 `user_acceptance=pending`；
...
M8A–M8E、D-055、M8C-1 与 RC.2 root-layout correction 已完成工程实现和候选自动验证。用户对 RC.2 的下一轮验收产生 D-084–D-087；RC.3 candidate gate 已完成，当前下一动作是用户独立验收。阶段结果为：
...
7. RC.3：Assessment 技术执行、图标、全局删除与按住拖动、候选文档、截图、annotated tag、ZIP 与仓库外 gate 均已完成；该历史候选保持不可变且 `user_acceptance=pending`。
8. RC.3 后续 UAT 返修：D-088 主动“保存全部”和 D-089 节点/Raw Input Family 拖拽持久化已在 `main` 完成代码、轻量回归、保存/重启和可见预览验证；尚未建立新的 tagged candidate，等待本轮用户检查收口后统一重建候选文档与包。
