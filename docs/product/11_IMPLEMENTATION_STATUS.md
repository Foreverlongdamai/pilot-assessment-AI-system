...
| M4 当前状态 | Replacement Task 0–26 已完成。`2d223a7` 新增 H1 AOI Dwell 与 `gaze-aoi-intervals-v1` provider：按 gaze 时刻关联真实第一人称 scene frame，支持静态/动态 2D、静态/动态 3D 与 `other_scene`，以 interval duration 计算加权 AOI dwell；动态几何按帧预索引，不逐 gaze 扫描全表。Task 26 focused/受控相关 gates 分别为 `14 passed`、`212 passed`；registry verify、Ruff/format、ty/diff 均通过。Task 20 的 `1275 passed, 3 skipped` 及 build/isolated-wheel 仍是最新 full stage-gate 证据，本单 anchor 未重复重门。按用户节省额度的明确要求全程 INLINE，未走独立审查子代理。packaged registry 现有且仅有 O1–O12/H1，capability count 为 13/18，preprocessor count 为 2（`movement-events-v1`、`gaze-aoi-intervals-v1`）；registry fingerprint 为 `51b3bc525b5647513852680d1b0fd5c5f49bc63cf31cb48e531880279024b434`，H1 digest 为 `4de79001cff14d43dd6f64350a4734762a9abd92b79c93d7b74f30f7bfa20350`，gaze provider digest 为 `079f9216918449c179e81f13442328b8aaf7766c6f9d33b995c589a8a1d2b8b0`。因此准确口径是 **M4-B framework engineering-verified；M4-C/M4-D software-verified；M4-E in progress；18/18 specified、13/18 production plugins；M4 整体尚未 engineering verified；`formal_run_authorized=false`** |
| 下一里程碑 | 执行 replacement Task 27，实现并验证 H2 First Fixation Latency 与 `fixation-intervals-v1` provider，继续 M4-E |
| 软件状态 | `in_progress`（M1/M2/M3 engineering verified；M4 Tasks 0–26 complete、M4-C/M4-D closed、M4-E in progress、O1–O12/H1 与两个 preprocessing providers available、Task 27 next；完整 Assessment Core alpha 与 Gate B 尚未完成） |
...
- M4：O1–O12/H1 已实现；其余 5 个 production AnchorPlugin、plugin-specific window materialization 与 raw availability 仍待实现；`AnchorResultV2`、catalog/dependency/execution-plan/immutable-request、measurement/artifact/inventory/report contracts、v0.2/M4 schema export、packaged exact-18 catalog/24 parameter resources、canonical fingerprints/runtime identity、trusted registry closure、通用 temporal/event/gaze-AOI primitives、transactional artifact sink、central scorer、typed DAG、preprocessing resolver、evaluator、public API、O1–O12/H1 plugins 与两个 preprocessing providers 已实现，当前为 18/18 specified、13/18 production plugins 已实现；
- M4 replacement plan：Task 0–26 已完成；M4-C/M4-D 已关闭、M4-E 进行中。O1 由 `b1d1fc9` 实现，O2 由 `b1a8743` 实现，O3 由 `f7d5261` 实现，O4 由 `056d9d5` 实现，O5 与 `movement-events-v1` provider 由 `1a119af` 实现，O6 由 `2ca3540` 实现，O7 由 `eb9cca6` 实现，O8 由 `15da2ea` 实现，O9 由 `db2b5da` 实现，O10 与共享 causal boolean primitive 由 `87aed10` 实现，O11 与共享 trailing causal median 由 `8672c74` 实现，O12 由 `e41d0e6` 实现并关闭 M4-D，H1 与 `gaze-aoi-intervals-v1` provider 由 `2d223a7` 实现并启动 M4-E；下一步执行 Task 27 H2；不得把 M4-D 分段完成或 M4-E 启动误计为 M4 整体 engineering verification；
...
下一步是依据已批准的 replacement implementation plan 执行 Task 27：实现并验证 H2 First Fixation Latency 与 `fixation-intervals-v1` provider，继续 M4-E。Tasks 8–13 已依次关闭 canonical identity、trusted packaged registry/implementation closure、segment-aware temporal primitives、transactional artifact publication、central scoring，以及 typed DAG/preprocessing/evaluator/public API；Task 14–26 已完成 O1–O12/H1、共享 event/gaze-AOI primitives 与两个 preprocessing providers，M4-B framework 与 M4-C/M4-D stage gates 已关闭，M4-E 进行中。Production AnchorPlugin 当前为 13/18，M4 整体尚未 engineering verified，更不能提前进入 BN、runner 或 WinUI。Replacement plan 后续覆盖：
...
原逐任务实施计划已被取代、仅供历史追溯；replacement plan 与 Task 3/7/8 amendments 已于 2026-07-13 获明确或授权默认批准，Task 0–26 已完成，M4-C/M4-D 已关闭、M4-E 进行中，下一步为 Task 27：
...
- [M4 Anchor Calculation and Evidence Availability Replacement Implementation Plan](plans/2026-07-13-m4-anchor-evidence-availability-replacement-implementation-plan.md)（已批准并按方案 A 修订；Task 0–26 已完成，M4-B/M4-C/M4-D gates 已关闭、M4-E 进行中，O1–O12/H1、共享 primitives 与两个 providers available，下一步为 Task 27 H2）
