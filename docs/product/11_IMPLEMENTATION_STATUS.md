# Implementation Status — RC.2 Root-Layout Candidate Externally Verified, User Acceptance Pending

| 字段 | 当前值 |
|---|---|
| 状态日期 | 2026-07-21 |
| 产品设计基线 | v0.1.0-rc.2 portable expert-designer release candidate（D-031–D-083） |
...
| 下一里程碑 | 用户独立验收完整 `v0.1.0-rc.2`；如仍发现问题，形成后续候选而不移动 RC.1/RC.2 标签 |
| 软件状态 | `in_progress`（M1–M8E engineering history preserved；RC.1 user acceptance=`changes-required`；clean tagged RC.2 已通过仓库外 restricted-PATH verification，`user_acceptance=pending`；starter/synthetic `formal_run_authorized=false`） |
...
M1/M2/M3、M4R、M5、M6、M7、M8A、M8B、M8C-1、M8D、D-055 与 M8E 的 engineering history 保持完整。`v0.1.0-rc.1` 已通过自动隔离验证，但用户手工验收确认根目录 94 folders / 374 files 不可接受，因此结论为 `changes-required`。clean annotated `v0.1.0-rc.2` 已实现 `app/` 收纳、唯一根启动器与对应 locator/manifest/verifier 修订，并在仓库外实测为 8 directories / 2 files / 1 launcher；用户验收重新保持 `pending`。系统当前工程实现可以：
...
RC.2 engineering candidate 已准备好供用户复验；`user_acceptance=pending` 与 `formal_run_authorized=false` 均保持不变。
...
M8A–M8E、D-055、M8C-1 与 RC.2 root-layout correction 已完成工程实现和候选自动验证。D-078–D-083 把下一动作确定为由用户独立验收完整 RC.2；候选在实际接受前保持 `user_acceptance=pending`。阶段结果为：
...
6. M8E/RC.2：RC.1 历史验证保留且用户结论为 `changes-required`；annotated `v0.1.0-rc.2`、8/2/1 根布局、checksums/SBOM/licenses/source identity、自动隔离/offline vertical slices 与真实 root-launcher 桌面启动均已完成；RC.2 用户验收作为候选后的独立状态。
...
- [RC.2 Portable Root Layout Amendment](specs/2026-07-21-rc2-portable-root-layout-amendment.md)、[Plan](plans/2026-07-21-rc2-portable-root-layout-implementation-plan.md)、[RC.1 User Acceptance Result](reviews/2026-07-21-v0.1.0-rc.1-user-acceptance-result.md) 与 [RC.2 Verification](reviews/2026-07-21-rc2-portable-root-layout-verification.md)（当前根布局、唯一启动器、新候选与 fresh evidence）
