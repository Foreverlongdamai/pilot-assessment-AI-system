...
| 下一里程碑 | M8B-2 新 Python operator 扩展、私有依赖工具、通用前端参数配置与维护手册闭环 |
| 软件状态 | `in_progress`（M1–M8B-1 engineering verified；M7 user acceptance pending；D-055、M8B-2 与 M8C–M8E 未完成；starter/synthetic `formal_run_authorized=false`） |
...
M1/M2/M3、M4R、M5、M6、M7、M8A、M8B-0 与 M8B-1 已实现并关闭各自 engineering gate；M7 用户手工验收尚未完成，并可能产生返修。系统当前工程实现可以：
...
- M7 是专家工作区工程完成；其首个 bundled portable runtime 已由 M8A 提供。完整 handoff、备份迁移、分类手册与 clean-machine final candidate 仍属于 M8B–M8E，真实专家校准与科学验证不属于软件 engineering gate。
...
- portable package 携带 clean starter `system/` baseline，不携带任何用户 project/session/result；一套软件副本只允许一个 system writer。
...
M6 收尾另行核对并关闭：project locator 不保存绝对根路径；managed import 不保留 external 运行依赖；SQLite domain mutation 可加入同一 idempotency/audit transaction；draft graph/layout batch 只保存一次；stdout 只有 JSON-RPC；大型 payload 只返回 project-relative/artifact reference；运行总量由 frozen Evidence closure 动态计算；sidecar adapters 没有复制 Evidence/BN 算法。M8A 关闭首个便携打包边界，M8B-0 关闭 software-copy system model ownership，M8B-1 关闭 loaded source provenance 与 source artifact；仍未实现的 M8B-2、M8C–M8E 与科学校准保留在 §5，不以工程测试冒充完成。

## 5. 尚未实现

- M8B-2：验证从发布副本新增最小 Python operator、注册、重启、前端配置与 run 的完整扩展闭环，并形成详细维护手册；
- M8C：分类 Markdown 权威原稿、自动生成的版本化 DOCX、截图/交叉引用/一致性检查与《系统技术参考总册》；
- M8D：项目 backup/restore/migration、source-aware historical replay 与默认脱敏 diagnostics；
...
M8A、M8B-0 与 M8B-1 已完成。下一开发里程碑是 M8B-2：围绕已冻结的 source identity 与 release layout，关闭依赖/新增 operator 开发工作流。M7 用户验收可与 M8B/M8C-0 工程工作并行，但必要返修与 D-055 必须在 M8E final candidate 前关闭。后续工作流为：
...
3. M8B-1：loaded source identity/snapshot（**已完成**）；M8B-2：新 operator 闭环和详细修改手册；
4. M8C：以 Markdown 为权威原稿、自动生成版本化 DOCX 的十二类文档和技术总册；
5. M8D：围绕现有 managed project 的 backup/restore/migration、RunSnapshot source identities/artifacts 和脱敏 diagnostics；
