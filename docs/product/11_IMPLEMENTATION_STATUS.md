# Implementation Status — M1/M2/M3/M4R/M5/M6/M7A Engineering Verified; M7B Task 1–13 Complete
...
| 下一里程碑 | 从 M7B Task 14 开始实现实际运行、结果、trace 与 diagnostics 工作区；M8 packaging 仍放在最后阶段 |
| 软件状态 | `in_progress`（M1/M2/M3/M4R/M5/M6/M7A engineering verified；M7B Task 1–13 已完成，运行结果、M7 completion gate 与 M8 packaging 尚未完成，starter/synthetic `formal_run_authorized=false`） |
...
M7A 已按 12-task INLINE plan 完成，current-model/automatic-snapshot 后端门已关闭。M7B Task 1–13 现已完成 WinUI 工程、强类型合同、受监督 sidecar、真实应用 shell、受管 project/session 工作区、canonical task-scheme navigation、backend-driven global active/dim graph、图上编辑、可并行打开并恢复位置的独立节点窗口、Raw Input/Evidence/BN/CPT 编辑器、持久 autosave/canonical reconciliation/conflict recovery，以及不触发后端写入的即时中英文切换；下一步从 Task 14 开始实现 actual run/results/trace/diagnostics。普通表单修改已写回 Python canonical state；离散 activation/edge/copy/archive 与 state/parent/CPT 操作继续走后端原子事务。`model.preview.node` 当前展示 frozen snapshot metadata/trace references，不能误写成已执行测量结果，实际 run/posterior/results 属于 Task 14。不能把它重新编号为 M6、回到旧 fixed-plugin Task 29，或提前做 M8 installer。M7B 必须消费 M7A current DTO，而不是把旧 draft/publish DTO 重新带回正常 UI：
...
- [M7 WinUI Expert Designer and Task Activation Workspace Design](specs/2026-07-17-m7-winui-expert-designer-and-task-activation-workspace-design.md)（当前产品语义；M7A 后端已实施，M7B Task 1–13 已完成）
...
- [M7B WinUI Expert Designer Implementation Plan](plans/2026-07-17-m7b-winui-expert-designer-implementation-plan.md)（当前 15 个 INLINE 前端任务；Tasks 1–13 已完成，Task 14 next）
