# Implementation Status — M1/M2/M3/M4R/M5/M6/M7A Engineering Verified; M7B Task 1–3 Complete
...
| 下一里程碑 | 从 M7B Task 4 开始组合 WinUI host、导航 shell 与状态服务，再逐步接入专家工作区；M8 packaging 仍放在最后阶段 |
| 软件状态 | `in_progress`（M1/M2/M3/M4R/M5/M6/M7A engineering verified；M7B Task 1–3 已完成，M7B 其余交互与 M8 packaging 尚未完成，starter/synthetic `formal_run_authorized=false`） |
...
M7A 已按 12-task INLINE plan 完成，current-model/automatic-snapshot 后端门已关闭。M7B Task 1–3 现已完成 WinUI 工程、强类型合同、JSONL/JSON-RPC client 与受监督 sidecar 生命周期；下一步从 Task 4 开始把已验证的 `model.*` API 接成真正可操作的 Windows 软件。不能把它重新编号为 M6、回到旧 fixed-plugin Task 29，或提前做 M8 installer。M7B 必须消费 M7A current DTO，而不是把旧 draft/publish DTO 重新带回正常 UI：
...
- [M7 WinUI Expert Designer and Task Activation Workspace Design](specs/2026-07-17-m7-winui-expert-designer-and-task-activation-workspace-design.md)（当前产品语义；M7A 后端已实施，M7B Task 1–3 已完成）
