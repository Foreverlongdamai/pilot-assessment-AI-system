...
| M4 当前状态 | Replacement Task 0–10 已分别由 `bc544bf`、`f56365c`、`928e9a4`、`e054620`、`1528d09`、`b63d38b`、`93c4ddb`、`583a1e7`、`3e1a006`、`cac645f`、`6db08c9` 完成；Task 8 canonical identity、Task 9 trusted packaged registry/implementation closure 与 Task 10 segment-aware temporal primitives 均已落地。Task 10 inline focused gate 为 `25 passed`，Ruff/format/ty clean；为节省额度未重复跑全仓、未走独立审查子代理，最近一次全仓证据仍为 Task 9 的 `1062 passed, 3 skipped`。`registry-v1.json` 仍为空，18 个 reference capability 全部 `not_implemented`，provider entry 不计入 18-anchor 基数；O1–O13、H1–H5 共 18/18 specified、0/18 production plugins；M4 尚未 engineering verified，`formal_run_authorized=false` |
| 下一里程碑 | 执行 replacement Task 11，实现 transactional content-addressed artifact publication |
| 软件状态 | `in_progress`（M1/M2/M3 engineering verified；M4 Tasks 0–10 complete、Task 11 next；完整 Assessment Core alpha 与 Gate B 尚未完成） |
...
- M4：18 个 AnchorPlugin、plugin-specific window materialization、evidence scorer、raw availability、artifact execution 与 O8/O13 派生证据；`AnchorResultV2`、catalog/dependency/execution-plan/immutable-request、measurement/artifact/inventory/report contracts、v0.2/M4 schema export、packaged exact-18 catalog/24 parameter resources、canonical fingerprints/runtime identity、trusted registry closure 与通用 temporal primitives 已实现，但当前仍为 18/18 specified、0/18 production plugins 已实现；
- M4 replacement plan：Task 0–10 已分别由 `bc544bf`、`f56365c`、`928e9a4`、`e054620`、`1528d09`、`b63d38b`、`93c4ddb`、`583a1e7`、`3e1a006`、`cac645f`、`6db08c9` 完成；下一步执行 Task 11 transactional artifact publication，不得把 framework primitive 完成误计为 production AnchorPlugin 或 M4 engineering verification；
...
下一步是依据已批准的 replacement implementation plan 执行 Task 11：实现 transactional content-addressed artifact publication。Tasks 8–10 已依次关闭 canonical identity、trusted packaged registry/implementation closure 和 segment-aware temporal primitives；18 个 production AnchorPlugin 仍为 0/18。M4-B 尚未关闭，更不能提前进入 BN、runner 或 WinUI。Replacement plan 后续覆盖：
...
原逐任务实施计划已被取代、仅供历史追溯；replacement plan 与 Task 3/7/8 amendments 已于 2026-07-13 获明确或授权默认批准，Task 0–10 已完成，下一步为 Task 11：
...
- [M4 Anchor Calculation and Evidence Availability Replacement Implementation Plan](plans/2026-07-13-m4-anchor-evidence-availability-replacement-implementation-plan.md)（已批准并按方案 A 修订；Task 0–10 已完成，下一步为 Task 11 transactional artifact publication）
