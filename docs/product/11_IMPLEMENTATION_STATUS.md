...
| M4 当前状态 | Replacement Task 0–27 已完成。`ed9cf24` 新增 H2 First Fixation Latency 与 `fixation-intervals-v1` provider：从 authoritative raw G 以 shortest-sphere I-VT 重算 fixation，稳定首行处理重复时间戳，使用 inclusive 100 deg/s 与 100 ms 边界，忽略兼容/设备 `G.fixations` 表；H2 从 cue available 时刻计时、裁到 2 s/session/opportunity end，任一事件 miss 否决 session 且保存 finite wait。Task 27 focused/受控相关 gates 分别为 `18 passed`、`231 passed`；registry verify、Ruff/format、ty/diff 均通过。Task 20 的 `1275 passed, 3 skipped` 及 build/isolated-wheel 仍是最新 full stage-gate 证据，本单 anchor 未重复重门。按用户节省额度的明确要求全程 INLINE，未走独立审查子代理。packaged registry 现有且仅有 O1–O12/H1/H2，capability count 为 14/18，preprocessor count 为 3；registry fingerprint 为 `90df48976a6b4b508f24ab58827a2266a35a7df2be4a18ce735ae6ac02366389`，H2 digest 为 `49a179b7ad08e46bce611f7f418c837b1fc8722608546bd67dc3f4b18547ae1f`，fixation provider digest 为 `e3751b783b37ff0e1c6cc68dfdd752b6c38ca92a434c3b38373a0cae5a8fe72a`。因此准确口径是 **M4-B framework engineering-verified；M4-C/M4-D software-verified；M4-E in progress；18/18 specified、14/18 production plugins；M4 整体尚未 engineering verified；`formal_run_authorized=false`** |
| 下一里程碑 | 执行 replacement Task 28，实现并验证 H3 Off-task Dwell，关闭 M4-E |
| 软件状态 | `in_progress`（M1/M2/M3 engineering verified；M4 Tasks 0–27 complete、M4-C/M4-D closed、M4-E in progress、O1–O12/H1/H2 与三个 preprocessing providers available、Task 28 next；完整 Assessment Core alpha 与 Gate B 尚未完成） |
...
- M4：O1–O12/H1/H2 已实现；其余 4 个 production AnchorPlugin、plugin-specific window materialization 与 raw availability 仍待实现；`AnchorResultV2`、catalog/dependency/execution-plan/immutable-request、measurement/artifact/inventory/report contracts、v0.2/M4 schema export、packaged exact-18 catalog/24 parameter resources、canonical fingerprints/runtime identity、trusted registry closure、通用 temporal/event/gaze-AOI/fixation primitives、transactional artifact sink、central scorer、typed DAG、preprocessing resolver、evaluator、public API、O1–O12/H1/H2 plugins 与三个 preprocessing providers 已实现，当前为 18/18 specified、14/18 production plugins 已实现；
- M4 replacement plan：Task 0–27 已完成；M4-C/M4-D 已关闭、M4-E 进行中。O1 由 `b1d1fc9` 实现，O2 由 `b1a8743` 实现，O3 由 `f7d5261` 实现，O4 由 `056d9d5` 实现，O5 与 `movement-events-v1` provider 由 `1a119af` 实现，O6 由 `2ca3540` 实现，O7 由 `eb9cca6` 实现，O8 由 `15da2ea` 实现，O9 由 `db2b5da` 实现，O10 与共享 causal boolean primitive 由 `87aed10` 实现，O11 与共享 trailing causal median 由 `8672c74` 实现，O12 由 `e41d0e6` 实现并关闭 M4-D，H1 与 `gaze-aoi-intervals-v1` provider 由 `2d223a7` 实现并启动 M4-E，H2 与 `fixation-intervals-v1` provider 由 `ed9cf24` 实现；下一步执行 Task 28 H3 并关闭 M4-E；不得把 M4-D 分段完成或 M4-E 进行中误计为 M4 整体 engineering verification；
...
下一步是依据已批准的 replacement implementation plan 执行 Task 28：实现并验证 H3 Off-task Dwell，关闭 M4-E。Tasks 8–13 已依次关闭 canonical identity、trusted packaged registry/implementation closure、segment-aware temporal primitives、transactional artifact publication、central scoring，以及 typed DAG/preprocessing/evaluator/public API；Task 14–27 已完成 O1–O12/H1/H2、共享 primitives 与三个 preprocessing providers，M4-B framework 与 M4-C/M4-D stage gates 已关闭，M4-E 进行中。Production AnchorPlugin 当前为 14/18，M4 整体尚未 engineering verified，更不能提前进入 BN、runner 或 WinUI。Replacement plan 后续覆盖：
...
原逐任务实施计划已被取代、仅供历史追溯；replacement plan 与 Task 3/7/8 amendments 已于 2026-07-13 获明确或授权默认批准，Task 0–27 已完成，M4-C/M4-D 已关闭、M4-E 进行中，下一步为 Task 28：
...
- [M4 Anchor Calculation and Evidence Availability Replacement Implementation Plan](plans/2026-07-13-m4-anchor-evidence-availability-replacement-implementation-plan.md)（已批准并按方案 A 修订；Task 0–27 已完成，M4-B/M4-C/M4-D gates 已关闭、M4-E 进行中，O1–O12/H1/H2、共享 primitives 与三个 providers available，下一步为 Task 28 H3）
