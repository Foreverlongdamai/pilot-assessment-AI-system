# Implementation Status — M1/M2/M3 Engineering Verified; M4 Tasks 0–16 Complete
...
| M4 当前状态 | Replacement Task 0–17 已完成。`b1d1fc9` 实现 O1，`b1a8743` 实现稳定 time-aligned reference join 与 O2，`f7d5261` 实现 gap-aware capture envelope、O3 production plugin 与 event-identity contract correction，`056d9d5` 实现 native-rate stable-hover kernel、O4 production plugin 与 effective stable mask。Task 17 focused/related/轻量 M4 gate 分别为 `19 passed`、`141 passed`、`755 passed, 1 skipped`；Task 16 的 `1210 passed, 3 skipped` 仍是最新全仓证据，registry verify、schema export 零漂移、Ruff/format/ty/build/isolated-wheel 均通过。按用户节省额度的明确要求全程 INLINE，未走独立审查子代理。packaged registry 现有且仅有 O1/O2/O3/O4，capability count 为 4/18，preprocessor count 为 0；因此准确口径是 **M4-B framework engineering-verified；18/18 specified、4/18 production plugins；M4-C 与 M4 整体尚未 engineering verified；`formal_run_authorized=false`** |
| 下一里程碑 | 执行 replacement Task 18，实现并验证 O5 Workload Rate |
| 软件状态 | `in_progress`（M1/M2/M3 engineering verified；M4 Tasks 0–17 complete、M4-B framework engineering-verified、O1/O2/O3/O4 available、Task 18 next；完整 Assessment Core alpha 与 Gate B 尚未完成） |
...
下一步是依据已批准的 replacement implementation plan 执行 Task 18：实现并验证 O5 Workload Rate。Tasks 8–13 已依次关闭 canonical identity、trusted packaged registry/implementation closure、segment-aware temporal primitives、transactional artifact publication、central scoring，以及 typed DAG/preprocessing/evaluator/public API；Task 14/15/16/17 已完成 O1/O2/O3/O4。M4-B framework gate 已关闭，但 production AnchorPlugin 仅为 4/18，M4-C 与 M4 整体尚未 engineering verified，更不能提前进入 BN、runner 或 WinUI。Replacement plan 后续覆盖：
...
原逐任务实施计划已被取代、仅供历史追溯；replacement plan 与 Task 3/7/8 amendments 已于 2026-07-13 获明确或授权默认批准，Task 0–17 已完成，下一步为 Task 18：
...
- [M4 Anchor Calculation and Evidence Availability Replacement Implementation Plan](plans/2026-07-13-m4-anchor-evidence-availability-replacement-implementation-plan.md)（已批准并按方案 A 修订；Task 0–17 已完成，M4-B framework gate 已关闭，O1/O2/O3/O4 available，下一步为 Task 18 O5）
