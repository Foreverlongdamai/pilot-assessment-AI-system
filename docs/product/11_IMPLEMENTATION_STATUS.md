...
| 下一里程碑 | M5 Task 1–3 generic identity、public contracts/Schema、typed source provenance 与 M4R migration preflight 已完成；下一执行入口为 Task 4 global component library。后续范围仍为 exact-pinned task schemes、BN editing/inference、O8 新并行版本与 copy-on-write publish；不自动把 M6/M7 混入 M5 |
| 软件状态 | `in_progress`（M1/M2/M3/M4R engineering verified；M5 Task 1–3 已完成，但 global component repository/service、BN/model workspace 整体、M6 persistence/sidecar、M7 WinUI 与 M8 packaging 尚未完成） |
