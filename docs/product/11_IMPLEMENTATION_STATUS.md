# Implementation Status — M1/M2/M3 Engineering Verified; M4 Tasks 0–18 Complete
...
| M4 当前状态 | Replacement Task 0–18 已完成。`b1d1fc9` 实现 O1，`b1a8743` 实现稳定 time-aligned reference join 与 O2，`f7d5261` 实现 gap-aware capture envelope、O3 production plugin 与 event-identity contract correction，`056d9d5` 实现 native-rate stable-hover kernel、O4 production plugin 与 effective stable mask，`1a119af` 实现 exact movement detector、`movement-events-v1` provider 与 O5 production plugin。Task 18 focused/计划内/受控相关 gate 分别为 `18 passed`、`59 passed`、`261 passed`；Task 16 的 `1210 passed, 3 skipped` 仍是最新全仓证据，registry verify、schema export 零漂移、Ruff/format/ty/build/isolated-wheel 均通过。按用户节省额度的明确要求全程 INLINE，未走独立审查子代理。packaged registry 现有且仅有 O1/O2/O3/O4/O5，capability count 为 5/18，preprocessor count 为 1（`movement-events-v1`）；因此准确口径是 **M4-B framework engineering-verified；18/18 specified、5/18 production plugins；M4-C 与 M4 整体尚未 engineering verified；`formal_run_authorized=false`** |
| 下一里程碑 | 执行 replacement Task 19，实现并验证 O6 Control Magnitude RMS |
| 软件状态 | `in_progress`（M1/M2/M3 engineering verified；M4 Tasks 0–18 complete、M4-B framework engineering-verified、O1/O2/O3/O4/O5 与 `movement-events-v1` provider available、Task 19 next；完整 Assessment Core alpha 与 Gate B 尚未完成） |
...
- M4：O1/O2/O3/O4/O5 已实现；其余 13 个 production AnchorPlugin、plugin-specific window materialization、raw availability 与 O8/O13 派生证据仍待实现；`AnchorResultV2`、catalog/dependency/execution-plan/immutable-request、measurement/artifact/inventory/report contracts、v0.2/M4 schema export、packaged exact-18 catalog/24 parameter resources、canonical fingerprints/runtime identity、trusted registry closure、通用 temporal primitives、transactional artifact sink、central scorer、typed DAG、preprocessing resolver、evaluator、public API、O1/O2/O3/O4/O5 shared kernels/plugins 与 `movement-events-v1` provider 已实现，当前为 18/18 specified、5/18 production plugins 已实现；
- M4 replacement plan：Task 0–18 已完成；M4-B framework gate 已关闭，O1 由 `b1d1fc9` 实现，O2 由 `b1a8743` 实现，O3 由 `f7d5261` 实现，O4 由 `056d9d5` 实现，O5 与 `movement-events-v1` provider 由 `1a119af` 实现，下一步执行 Task 19 O6；不得把五个 plugin 完成误计为 M4-C 或 M4 整体 engineering verification；
...
下一步是依据已批准的 replacement implementation plan 执行 Task 19：实现并验证 O6 Control Magnitude RMS。Tasks 8–13 已依次关闭 canonical identity、trusted packaged registry/implementation closure、segment-aware temporal primitives、transactional artifact publication、central scoring，以及 typed DAG/preprocessing/evaluator/public API；Task 14/15/16/17/18 已完成 O1/O2/O3/O4/O5 与首个 preprocessing provider。M4-B framework gate 已关闭，但 production AnchorPlugin 仅为 5/18，M4-C 与 M4 整体尚未 engineering verified，更不能提前进入 BN、runner 或 WinUI。Replacement plan 后续覆盖：
...
原逐任务实施计划已被取代、仅供历史追溯；replacement plan 与 Task 3/7/8 amendments 已于 2026-07-13 获明确或授权默认批准，Task 0–18 已完成，下一步为 Task 19：
...
- [M4 Anchor Calculation and Evidence Availability Replacement Implementation Plan](plans/2026-07-13-m4-anchor-evidence-availability-replacement-implementation-plan.md)（已批准并按方案 A 修订；Task 0–18 已完成，M4-B framework gate 已关闭，O1/O2/O3/O4/O5 与 `movement-events-v1` provider available，下一步为 Task 19 O6）
