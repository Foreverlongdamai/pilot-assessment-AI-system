# Implementation Status — M1/M2/M3/M4R/M5/M6/M7A Engineering Verified; M7B Task 1–4 Complete
...
| 下一里程碑 | 从 M7B Task 5 开始接入受管 project/session 工作区，再逐步实现 task scheme、模型画布与节点编辑；M8 packaging 仍放在最后阶段 |
| 软件状态 | `in_progress`（M1/M2/M3/M4R/M5/M6/M7A engineering verified；M7B Task 1–4 已完成，M7B 其余交互与 M8 packaging 尚未完成，starter/synthetic `formal_run_authorized=false`） |
...
2026-07-17 用户进一步确认 D-047–D-053：每个可见节点是一个完整、独立、只有一个 current definition 的 `ModelNode`；任务差异通过复制/新建不同节点表达；`TaskScheme` 只保存 active selection 与 parent closure；切换任务以 active/dim 展示；copy/paste 默认只复制节点并引用原 fixed parents；启用 child 自动补齐 parents，停用有 downstream 的 parent 需继续/取消确认；多节点独立浮动窗口与中英文属于 M7；正常交互取消 Draft/Published/Apply/Publish，每个 run 自动冻结 immutable `RunSnapshot`。M7A 已完成该语义的 Python/SQLite/sidecar 后端：focused current-model gate `42 passed`、M4R/M5/M6 compatibility gate `151 passed`、full repository `1684 passed, 3 skipped`，51 类 schema 零漂移，Ruff/format/ty/build 与仓库外 wheel smoke 全通过。M7B Task 1 已建立正式 WinUI 3 x64 工程并通过真实顶层窗口启动验证；Task 2 已完成 Python canonical DTO 到不可变 C# records/enums、discriminated node definitions、source-generated snake_case JSON 与严格合同测试的映射。10 个小型 Python fixture 均由原 Pydantic 模型重验，focused contract `9/9`、完整桌面测试 `10/10`、x64 Debug build `0 warning / 0 error`。Task 3 已完成严格 UTF-8 JSONL framing、并发安全 request matching、notification/cancellation、same-transaction retry helper、开发环境定位器与隐藏 sidecar 进程宿主；focused protocol `12/12`、真实 sidecar lifecycle `1/1`、完整桌面测试 Unit `21/21` / Contract `2/2`、build `0 warning / 0 error`。Task 4 已组合 Host/DI、Mica/NavigationView shell、顶栏状态、startup diagnostics、原子非领域偏好和确定性 shutdown；shell state `4/4`、完整桌面 Unit `25/25` / Contract `2/2`、x64 Debug build `0 warning / 0 error`，真实可见窗口显示 backend `Ready`，关闭后应用和所有 sidecar 子进程均退出。下一项是 Task 5 project/session 工作区；旧 M5/M6 draft/publish APIs 仅继续作为 migration/replay 基础。
...
M7A 已按 12-task INLINE plan 完成，current-model/automatic-snapshot 后端门已关闭。M7B Task 1–4 现已完成 WinUI 工程、强类型合同、JSONL/JSON-RPC client、受监督 sidecar 生命周期与真实可见应用 shell；下一步从 Task 5 开始接入受管 project/session 工作区。不能把它重新编号为 M6、回到旧 fixed-plugin Task 29，或提前做 M8 installer。M7B 必须消费 M7A current DTO，而不是把旧 draft/publish DTO 重新带回正常 UI：
...
- [M7 WinUI Expert Designer and Task Activation Workspace Design](specs/2026-07-17-m7-winui-expert-designer-and-task-activation-workspace-design.md)（当前产品语义；M7A 后端已实施，M7B Task 1–4 已完成）
