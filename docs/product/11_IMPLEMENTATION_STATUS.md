...
| 下一里程碑 | M5 Task 1–6 generic identity、public contracts/Schema、typed source/M4R preflight、进程内 global component library、exact-pinned validation 与 scheme draft/atomic publish 已完成；下一执行入口为 Task 7 BN/CPT validation、materialization 与 migration。后续范围仍包括 exact inference 与 O8 新并行版本；不自动把 M6/M7 混入 M5 |
| 软件状态 | `in_progress`（M1/M2/M3/M4R engineering verified；M5 Task 1–6 已完成，但 BN/CPT/inference、starter workspace 整体、M6 durable persistence/sidecar、M7 WinUI 与 M8 packaging 尚未完成） |
...
- M5 剩余：BN/CPT validation/materialization/migration、exact inference、compliant TPX parallel version、Hover starter package 与轻量 workflow；Task 1–6 identity、public DTO/schema、source/M4R preflight、进程内 global component repository/service、exact-pinned validation 与 scheme draft/atomic publish 已完成；
