# Implementation Status — M1/M2/M3 Engineering Verified; M4 Tasks 0–22 Complete, M4-D In Progress
...
| M4 当前状态 | Replacement Task 0–22 已完成。`b1d1fc9`/`b1a8743`/`f7d5261`/`056d9d5`/`1a119af`/`2ca3540`/`eb9cca6`/`15da2ea`/`db2b5da` 分别实现 O1–O9，其中 `1a119af` 同时实现 `movement-events-v1` provider；`a76abde` 收口 O9 对 provider-owned movement 下幅值阈值的所有权，`e86a53d` 同步 package-level O1–O9 inventory honesty test。Task 22 focused/受控/package-honesty gates 分别为 `10 passed`、`237 passed`、`97 passed`；registry verify、Ruff/format/ty/diff 均通过。Task 20 的 `1275 passed, 3 skipped` 及 build/isolated-wheel 仍是最新 stage-gate 证据，本单 anchor 未重复重门。按用户节省额度的明确要求全程 INLINE，未走独立审查子代理。packaged registry 现有且仅有 O1–O9，capability count 为 9/18，preprocessor count 为 1（`movement-events-v1`）；因此准确口径是 **M4-B framework engineering-verified；M4-C software-verified；M4-D in progress；18/18 specified、9/18 production plugins；M4 整体尚未 engineering verified；`formal_run_authorized=false`** |
| 下一里程碑 | 执行 replacement Task 23，实现并验证 O10 Recovery Time 与 event primitive |
| 软件状态 | `in_progress`（M1/M2/M3 engineering verified；M4 Tasks 0–22 complete、M4-C closed、M4-D in progress、O1–O9 与 `movement-events-v1` provider available、Task 23 next；完整 Assessment Core alpha 与 Gate B 尚未完成） |
...
- M4：O1–O9 已实现；其余 9 个 production AnchorPlugin、plugin-specific window materialization 与 raw availability 仍待实现；`AnchorResultV2`、catalog/dependency/execution-plan/immutable-request、measurement/artifact/inventory/report contracts、v0.2/M4 schema export、packaged exact-18 catalog/24 parameter resources、canonical fingerprints/runtime identity、trusted registry closure、通用 temporal primitives、transactional artifact sink、central scorer、typed DAG、preprocessing resolver、evaluator、public API、O1–O9 shared kernels/plugins 与 `movement-events-v1` provider 已实现，当前为 18/18 specified、9/18 production plugins 已实现；
- M4 replacement plan：Task 0–22 已完成；M4-C 已关闭、M4-D 已开始。O1 由 `b1d1fc9` 实现，O2 由 `b1a8743` 实现，O3 由 `f7d5261` 实现，O4 由 `056d9d5` 实现，O5 与 `movement-events-v1` provider 由 `1a119af` 实现，O6 由 `2ca3540` 实现，O7 由 `eb9cca6` 实现，O8 由 `15da2ea` 实现，O9 由 `db2b5da` 实现，下一步执行 Task 23 O10；不得把 M4-D 分段进展误计为 M4 整体 engineering verification；
...
下一步是依据已批准的 replacement implementation plan 执行 Task 23：实现并验证 O10 Recovery Time 与 event primitive，继续 M4-D。Tasks 8–13 已依次关闭 canonical identity、trusted packaged registry/implementation closure、segment-aware temporal primitives、transactional artifact publication、central scoring，以及 typed DAG/preprocessing/evaluator/public API；Task 14–22 已完成 O1–O9 与首个 preprocessing provider，M4-B framework 与 M4-C stage gate 已关闭，M4-D 已开始。Production AnchorPlugin 当前为 9/18，M4 整体尚未 engineering verified，更不能提前进入 BN、runner 或 WinUI。Replacement plan 后续覆盖：
...
原逐任务实施计划已被取代、仅供历史追溯；replacement plan 与 Task 3/7/8 amendments 已于 2026-07-13 获明确或授权默认批准，Task 0–22 已完成，M4-C 已关闭、M4-D 已开始，下一步为 Task 23：
...
- [M4 Anchor Calculation and Evidence Availability Replacement Implementation Plan](plans/2026-07-13-m4-anchor-evidence-availability-replacement-implementation-plan.md)（已批准并按方案 A 修订；Task 0–22 已完成，M4-B/M4-C gate 已关闭、M4-D 已开始，O1–O9 与 `movement-events-v1` provider available，下一步为 Task 23 O10）
