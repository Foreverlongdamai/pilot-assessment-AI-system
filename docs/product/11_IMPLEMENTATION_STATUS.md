# Implementation Status — RC.4 Lightweight Candidate Build In Progress
...
| 软件状态 | `RC.4 candidate build in progress / user_acceptance=pending`（M1–M8E engineering history preserved；RC.1/RC.2/RC.3 user acceptance=`changes-required`；starter/synthetic `formal_run_authorized=false`） |
...
M8A–M8E、D-055、M8C-1 与 RC.2 root-layout correction 已完成工程实现和候选自动验证。用户对 RC.2 的验收产生 D-084–D-087；RC.3 gate 完成后，用户验收又产生 D-088–D-090；随后 D-091 收口 Model Studio 工具栏。当前下一动作是完成 RC.4 轻量 candidate 构建。阶段结果为：
...
8. RC.4：D-088 主动“保存全部”、D-089 节点/Raw Input Family 拖拽持久化和 D-091 工具栏本地化提示/按钮收口已在 `main` 完成代码、聚焦回归与可见预览验证；D-090 已分配新候选身份，轻量 tagged build 正在执行。
