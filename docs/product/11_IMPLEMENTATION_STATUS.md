# Implementation Status — M1/M2/M3 Engineering Verified; M4 Tasks 0–23 Complete, M4-D In Progress
...
| M4 当前状态 | Replacement Task 0–23 已完成。O1–O9 的实现与 O9 ownership/package-honesty 修复保持不变；`87aed10` 新增 O10 Recovery Time 与共享 `events` primitive。Task 23 focused/受控相关 gates 分别为 `17 passed`、`258 passed`；registry verify、Ruff/format（153 files）、ty/diff 均通过。Task 20 的 `1275 passed, 3 skipped` 及 build/isolated-wheel 仍是最新 stage-gate 证据，本单 anchor 未重复重门。按用户节省额度的明确要求全程 INLINE，未走独立审查子代理。packaged registry 现有且仅有 O1–O10，capability count 为 10/18，preprocessor count 为 1（`movement-events-v1`）；registry fingerprint 为 `1d8bcd84f4ea490abc1b83eba1f316249a3952ede4539960b965b34dfc13ec34`，O10 digest 为 `20dd10667c160e61c2246bf69d290013fbab267cfecd948b2f423cd337b162bf`。因此准确口径是 **M4-B framework engineering-verified；M4-C software-verified；M4-D in progress；18/18 specified、10/18 production plugins；M4 整体尚未 engineering verified；`formal_run_authorized=false`** |
| 下一里程碑 | 执行 replacement Task 24，复用共享 `events` primitive 实现并验证 O11 Disturbance Latency |
| 软件状态 | `in_progress`（M1/M2/M3 engineering verified；M4 Tasks 0–23 complete、M4-C closed、M4-D in progress、O1–O10 与 `movement-events-v1` provider available、Task 24 next；完整 Assessment Core alpha 与 Gate B 尚未完成） |
...
- M4：O1–O10 已实现；其余 8 个 production AnchorPlugin、plugin-specific window materialization 与 raw availability 仍待实现；`AnchorResultV2`、catalog/dependency/execution-plan/immutable-request、measurement/artifact/inventory/report contracts、v0.2/M4 schema export、packaged exact-18 catalog/24 parameter resources、canonical fingerprints/runtime identity、trusted registry closure、通用 temporal/event primitives、transactional artifact sink、central scorer、typed DAG、preprocessing resolver、evaluator、public API、O1–O10 shared kernels/plugins 与 `movement-events-v1` provider 已实现，当前为 18/18 specified、10/18 production plugins 已实现；
- M4 replacement plan：Task 0–23 已完成；M4-C 已关闭、M4-D 正在执行。O1 由 `b1d1fc9` 实现，O2 由 `b1a8743` 实现，O3 由 `f7d5261` 实现，O4 由 `056d9d5` 实现，O5 与 `movement-events-v1` provider 由 `1a119af` 实现，O6 由 `2ca3540` 实现，O7 由 `eb9cca6` 实现，O8 由 `15da2ea` 实现，O9 由 `db2b5da` 实现，O10 与共享 `events` primitive 由 `87aed10` 实现，下一步执行 Task 24 O11；不得把 M4-D 分段进展误计为 M4 整体 engineering verification；
...
下一步是依据已批准的 replacement implementation plan 执行 Task 24：复用共享 `events` primitive 实现并验证 O11 Disturbance Latency，继续 M4-D。Tasks 8–13 已依次关闭 canonical identity、trusted packaged registry/implementation closure、segment-aware temporal primitives、transactional artifact publication、central scoring，以及 typed DAG/preprocessing/evaluator/public API；Task 14–23 已完成 O1–O10、共享 event primitive 与首个 preprocessing provider，M4-B framework 与 M4-C stage gate 已关闭，M4-D 正在执行。Production AnchorPlugin 当前为 10/18，M4 整体尚未 engineering verified，更不能提前进入 BN、runner 或 WinUI。Replacement plan 后续覆盖：
...
原逐任务实施计划已被取代、仅供历史追溯；replacement plan 与 Task 3/7/8 amendments 已于 2026-07-13 获明确或授权默认批准，Task 0–23 已完成，M4-C 已关闭、M4-D 正在执行，下一步为 Task 24：
