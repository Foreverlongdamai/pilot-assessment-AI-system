# Implementation Status — M1/M2/M3 Engineering Verified; M4 Tasks 0–2 Complete
...
| M4 当前状态 | Replacement Task 0–2 已分别由 `bc544bf`、`f56365c`、`928e9a4` 完成；Task 3 方案 A 与 D-028 已批准并迁移，下一步为修订后的 Task 3；O1–O13、H1–H5 共 18/18 已设计、0/18 production plugins 已实现；M4 尚未 engineering verified |
| 下一里程碑 | 按三参数 candidate binder 修订执行 replacement Task 3，定义 semantic snapshot、resolved-reference snapshot 与 internal runtime binding |
| 软件状态 | `in_progress`（M1/M2/M3 engineering verified；M4 Tasks 0–2 complete、Task 3 next；完整 Assessment Core alpha 与 Gate B 尚未完成） |
...
- M4：18 个 AnchorPlugin、v0.2 JSON Schema/export、catalog/plan/report contracts、window grid、evidence scorer、raw availability、artifact/fingerprint 与 O8/O13 派生证据；`AnchorResultV2` typed contract 已实现，但当前仍为 18/18 已设计、0/18 production plugins 已实现；
- M4 replacement plan：Task 0 轻量 fixture 前置门、Task 1 numeric/JCS runtime 审计与 Task 2 breaking result contract 已分别由 `bc544bf`、`f56365c`、`928e9a4` 完成；Task 3 已按方案 A/D-028 修订为 session-bound candidate + 三参数 exact binder，下一步执行该修订，不得把合同完成误计为 production AnchorPlugin 或 M4 engineering verification；
...
下一步是依据已批准的 replacement implementation plan 和 Task 3 定向修订执行 Task 3：定义 semantic snapshot 与 resolved-reference snapshot contracts，包括 phase/event/AOI/control/baseline applicability、half-open intervals、唯一 ID、有限坐标/单位，以及 reference `present/absent`、session-bound candidate、M3 provenance、descriptor/checksum/frame 边界。Task 3 尚未开始；v0.2 JSON Schema/export 仍属于 Task 6。M4-A 尚未关闭，更不能提前进入 production AnchorPlugin、BN、runner 或 WinUI。Replacement plan 后续覆盖：
...
原逐任务实施计划已被取代、仅供历史追溯；replacement plan 与 Task 3 candidate-binding amendment 已于 2026-07-13 获用户批准，Task 0–2 已完成，下一步为修订后的 Task 3：
...
- [M4 Anchor Calculation and Evidence Availability Replacement Implementation Plan](plans/2026-07-13-m4-anchor-evidence-availability-replacement-implementation-plan.md)（已批准并按方案 A 修订；Task 0–2 已完成，下一步为修订后的 Task 3）
