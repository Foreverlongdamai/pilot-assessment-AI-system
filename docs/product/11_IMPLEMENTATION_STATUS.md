# Implementation Status — RC.4 Lightweight Engineering Verified

| 字段 | 当前值 |
|---|---|
| 状态日期 | 2026-07-21 |
| 产品设计基线 | v0.1.0-rc.4 portable expert-designer release candidate（D-031–D-091） |
...
| 下一里程碑 | 构建并轻量验证 annotated `v0.1.0-rc.4`，再交由用户独立验收；不移动 RC.1/RC.2/RC.3 标签 |
| 软件状态 | `RC.4 lightweight engineering verified / user_acceptance=pending`（M1–M8E engineering history preserved；RC.1/RC.2/RC.3 user acceptance=`changes-required`；starter/synthetic `formal_run_authorized=false`） |
...
M1/M2/M3、M4R、M5、M6、M7、M8A、M8B、M8C-1、M8D、D-055 与 M8E 的 engineering history 保持完整。RC.1 因根目录 94 folders / 374 files 被标记为 `changes-required`；RC.2 因 Assessment、图标、删除和拖动缺陷同样为 `changes-required`；RC.3 虽完成 full engineering gate，但用户实际操作继续发现无主动 Save All、普通节点松手回弹和绿色 Raw Input Family 不可拖动，因此也记录为 `changes-required`。D-088/D-089 已在 `main` 实现并通过聚焦测试与真实 WinUI 复测，D-090 将其纳入 RC.4；D-091 又关闭工具栏误报 `Ctrl+C` 及无反馈末尾按钮问题。按用户要求，RC.4 只做差异功能、包完整性和真实启动的轻量验证，不重复全仓与 281 页逐页验收。系统当前工程实现可以：
...
- M7 是专家工作区工程完成；其 bundled portable runtime 与 editable-source handoff 已由 M8A/M8B 提供，current-system packaging/project portability/compatibility Diagnostics 已由 M8D 提供，最终分类手册由 M8C-1 提供，clean-tagged candidate 的最终构建与外部验证已由 M8E Task 13 完成。专用 backup/restore 已取消，真实专家校准与科学验证不属于软件 engineering gate。
...
### 3.14 M8E release candidate 完成门

2026-07-21 `v0.1.0-rc.1` fresh release-candidate gate：

- clean annotated tag `v0.1.0-rc.1` 固定到 `c736bf7ad58bd24212b8997c5bbbf427b96e2692`；候选目录含 `4,332` 个文件、`793,023,050` bytes，其中 `4,331` 个文件进入 checksum manifest；
- ZIP 为 `287,355,423` bytes，SHA-256 `56cc4cf7ec95edb551626d620424080e52604cc06ad9b5462bb83d8796d7a5ff`；release manifest、SPDX SBOM、独立 `.sha256` 与 delivery JSON 均已生成；
- fresh gates 为 Python focused `132/132`、Desktop Unit Release `106/106`、real-sidecar Contract Release x64 `4/4`、Ruff lint/format、`ty check src` 与 x64 Release build `0 warning / 0 error`；
- 文档为 `24` 份 released DOCX、`10` 张 candidate screenshots、共 `277` 个渲染页；中英文技术总册分别为 `65 / 56` 页；documentation manifest SHA-256 为 `774ed8369d64b01a398c5e6f0f61925627b800bfbef8426099ff33bc5bef5f10`；
- 内部和仓库外 restricted-PATH verifier 均为 `PASS`：私有 CPython `3.11.9`、`16` 个依赖、`301` 个一方 backend source files、clean catalog `45` operators、临时扩展后 `46`，示例 recipe `2.75 / desired` 并完成 `201` 行轻量 run；WinUI 获得非零窗口句柄，sidecar stderr 为空；
- 构建前后 source system 保持同一 model identity `79efc59cb38242a7edfa1c85a5311729c40a769bcbf7256b2f5f1d5cb0400a1e`、`54 / 2`、DB schema `6`、system schema `1`、零 user-owned rows、相同文件 hashes 且无 WAL/SHM 或进程残留；
- ZIP 路径、提取文本和 `360` 个 DOCX XML 文件的隐私/内容扫描未发现开发机私有路径、用户 projects/sessions/results、cache、PDB 或未列入 manifest 的文件；所有验证性修改只发生在随后销毁的副本；
- 详细命令、工具链、故障修正和边界见 [M8E Release Candidate Verification](reviews/2026-07-21-m8e-release-candidate-verification.md)。

上述证据关闭 M8E engineering gate。它不关闭用户验收或科学校准；候选继续携带 `user_acceptance=pending` 与 `formal_run_authorized=false`。

### 3.15 RC.1 用户验收与 RC.2 根布局返修门

2026-07-21 用户把 `v0.1.0-rc.1` 标记为 `changes-required`：根目录暴露 `94` 个文件夹和 `374` 个文件。D-082/D-083 保留 RC.1 不可变，并以新候选修订物理布局。

`v0.1.0-rc.2` fresh gate：

- clean annotated tag 固定到 `865c3e993ac2c74cc8161fc5d65cf2e108d07001`；release manifest 升级为 v3，entrypoint 为根 `PilotAssessment.exe`，desktop executable 为 `app/PilotAssessment.Desktop.exe`；
- 实际 product root 为 `8` directories / `2` files / `1` launcher；目录只含 `app/backend/developer/docs/licenses/manifest/runtime/system`，文件只含 `PilotAssessment.exe/README.txt`；
- package 含 `4,334` files / `804,343,304` bytes，ZIP 为 `293,346,576` bytes，SHA-256 `ebc5978075f399063c2c6b40f2d0a2594635713d56475f977c181ba7d612610a`；
- fresh gates 为 Desktop Unit `107/107`、Contract `4/4`、release/documentation `28/28`、focused release `11/11`、x64 Release build `0 warning / 0 error` 和 `24/24` released DOCX；
- 仓库外 restricted-PATH 启动观察到 root launcher PID `43968`、`app/` desktop PID `22676`、非零 window handle `1969486` 与 root private-Python sidecar PID `44088`；同时通过 editable source、operator extension、lightweight run、零 TCP listener 和隐私扫描；
- 完整证据见 [RC.2 Portable Root Layout Verification](reviews/2026-07-21-rc2-portable-root-layout-verification.md)。

RC.2 的工程验证记录保持有效，但独立用户验收结论现为 `changes-required`。

### 3.16 RC.2 用户验收与 RC.3 交互返修门

用户在真实桌面操作中确认四项缺陷：Assessment 用途不能运行、发布任务栏图标缺失、无全局删除节点、长按拖动无可见移动。D-084–D-087 的 RC.3 实现已完成以下聚焦门：

- Assessment preflight 在技术 ready 时保持 ready，返回 `run.assessment_not_authorized` warning 与 `formal_run_authorized=false`，并能创建 Assessment-purpose snapshot；
- global archive 在一个后端 edit-session 事务中级联所有受影响 TaskScheme 并归档节点，C#/JSON-RPC/toolbar/context-menu 已贯通；
- WinUI Button handled pointer events 通过 `AddHandler(..., handledEventsToo: true)` 进入按住拖动；4 px movement threshold 区分普通点击，稳定坐标驱动可见 transform，不再依赖驻留计时器；
- `AppWindow.SetIcon` 使用 executable-relative absolute path，`Assets/AppIcon.ico` 进入 build/publish；
- 可见复测同时发现并关闭 runtime language toggle 的 `Microsoft.UI.Xaml.dll` crash：切换仅替换显式 resource context，不再写进程级 platform language override；
- fresh focused Python 为 `5 passed`，cross-layer Python 为 `64 passed`；最终完整 Python 回归为 `1760 passed, 3 skipped`，Desktop Unit 为 `111/111`，Contract 为 `4/4`，x64 Release build 为 `0 warning / 0 error`，Ruff/format/ty 全部通过。三项 skip 仅对应两个未配置的仓库外真实样例路径和一个显式 current-system release-source gate。
- 24 份 released DOCX 已重新生成并逐页检查全部 `281` 个渲染页；documentation manifest SHA-256 为 `7e3a8118164808fdf1a2803ee2bc78a0f542354534b0dd1ddac94ad49530ea50`。Model Studio 中英文截图已按最终 UI source identity `c75682021ab54b74b331b4c602f13b4a7be7f7625855464849845c90b406b85a` 重新捕获，其余无关界面截图携带显式 RC.2 reuse provenance。

Annotated `v0.1.0-rc.3` 指向 `5d44c4becce496b418ffeef76d28d27e11fc33d1`。ZIP SHA-256 为 `15be20fca5e38c5dff0b3e698c8144ed166c20610c87979562d3dce0b069ef72`；仓库外 restricted-PATH verifier 为 `PASS`，Assessment run 为 `completed` 并保留 authorization warning / `formal_run_authorized=false`。RC.3 engineering candidate ready，但用户验收仍为 `pending`。

### 3.17 RC.3 后续用户检查：主动“保存全部”返修

2026-07-22 用户要求无需关闭软件即可保存全部模型修改。D-088 已在 `main` 的后续源码中实现：

- 主 CommandBar 增加本地化、可访问的“保存全部”，`Ctrl+S` 走同一操作；
- 保存前 flush 全部浮动节点编辑、CPT/参数、pending graph layout 与 placement；dirty 时复用既有 `model.edit.commit`，不复制 Python 事务逻辑；
- commit 后重新加载 system task schemes 和当前 graph，使已打开的图与浮动节点窗口 rebase 到正式 revision；
- 节点、边、CPT、任务方案和布局 mutation 成功后统一显示“有待确认的暂存修改”，主动 commit 后显示“已保存到系统模型”，不再误写“已暂存到项目”；
- clean save 不创建空提交；保存失败保留 edit session；主动保存和关闭流程用同一 gate 串行化；关闭时 Save/Discard/Cancel 语义不变；
- fresh Desktop Unit 为 `112/112`，real-sidecar Contract 为 `4/4`，x64 Release build 为 `0 warning / 0 error`；
- 可见临时副本实测先停用 O6、主动保存并在无 dirty prompt 下关闭，再用最新 build 重新启用 O6；mutation 后显示 pending，点击“保存全部”后显示“已保存到系统模型”，图重载仍为 O6 active 且窗口保持打开。

本返修首先存在于 RC.3 tag 之后的 `main` 与 disposable preview；`v0.1.0-rc.3` tag、历史 ZIP identity、checksum 和既有验证记录保持不可变。D-090 随后把它纳入新的 RC.4 candidate sequence。详细边界见 [Active Save All Verification](reviews/2026-07-22-active-save-all-verification.md)。

### 3.18 RC.3 后续用户检查：节点松手回弹与 Raw Input Family 不可拖动返修

2026-07-22 用户继续检查实际 WinUI 画布，确认普通节点拖动预览存在但松手回到原点，五个绿色 Raw Input Family 根完全没有拖动入口。D-089 已在 `main` 的后续源码中关闭：

- pointer delta 改用 XamlRoot 稳定坐标；按下时捕获 projection，避免 `ItemsRepeater` 虚拟化令 release 时 `Node == null`；release/cancel 只完成一次；
- `raw-family.X/U/I/G/P` 接入同一拖拽生命周期，并作为 TaskScheme display-only layout override 暂存和保存；
- family 根仍不是 ModelNode，不进入 Evidence/BN、CPT、activation closure 或 RunSnapshot 计算语义；
- fresh Desktop Unit 为 `113/113`，real-sidecar Contract 为 `4/4`，相关 Python service 为 `8/8`，Ruff 与 x64 self-contained publish 通过；
- 最终可见副本实测 canonical `G.frames` 和绿色 `X(t)` 均在松手及 debounce 后保持新位置，staging layout revision 正确递增，“保存全部”成功，重启后从 system model 恢复坐标。

本返修与 D-088 一样首先落在 RC.3 之后的 `main`，不可改写 RC.3 历史候选；D-090 随后把它纳入 RC.4。详细证据见 [Node Drag Persistence Verification](reviews/2026-07-22-node-drag-persistence-verification.md)。

### 3.19 RC.3 用户验收与 RC.4 轻量差异发布门

RC.3 用户验收结论为 `changes-required`，精确事实见 [RC.3 User Acceptance Result](reviews/2026-07-22-v0.1.0-rc.3-user-acceptance-result.md)。RC.4 只重新验证 D-088/D-089/D-091 差异、x64 Release build、文档身份/结构、ZIP/hash/privacy/root layout 和真实根启动器；M8E/RC.3 已通过的全仓、editable-source、operator-extension、assessment-run 与 281 页视觉证据作为历史基础复用，不作为 RC.4 单独重复验收。该轻量门已通过：tagged commit `e14232f448a9ef9609860b894ea26fba43d59ba8`，ZIP SHA-256 `a5fe85b6ce24dab76acff1d6b840a9fa2b5498e7f511ebb56d228db1c422f7f4`；精确证据见 [RC.4 Lightweight Verification](reviews/2026-07-22-v0.1.0-rc.4-lightweight-verification.md)。
...
M7A 与 M7B 原计划、D-054–D-061 用户验收返修和 D-055 单一英文 current-model contract/persistence migration 均已工程实现，但用户尚未验收完整候选。产品显示层只呈现语义名称并隐藏 fallback marker/普通技术 ID；current model canonical content 只保存英文单字段，历史双语 records 仅作 replay。Session Import 已支持 canonical/raw source 自动识别与受管物化。普通表单、离散 activation/edge/copy/archive 与 state/parent/CPT 操作都先写入 Python 持久 edit session；关闭主窗口时统一保存全部或放弃全部。dirty 草稿不得运行；clean canonical scheme 继续自动冻结 immutable RunSnapshot，无业务 Publish 步骤。

M8A–M8E、D-055、M8C-1 与 RC.2 root-layout correction 已完成工程实现和候选自动验证。用户对 RC.2 的验收产生 D-084–D-087；RC.3 gate 完成后，用户验收又产生 D-088–D-090；随后 D-091 收口 Model Studio 工具栏。RC.4 轻量 candidate 已完成构建和仓库外验证，当前下一动作是用户/导师对精确 ZIP 进行独立验收。阶段结果为：
...
4. M8C：catalog/schema、固定工具链、DOCX pipeline、C4、11 类双语模块正文、24 份候选 DOCX 与 10 张 candidate screenshots（**已完成**）；
5. M8D：current-system packaging、project directory portability、RunSnapshot/source identity continuity 与 diagnostics（**已完成**）；
6. M8E/RC.2：RC.1 历史验证保留；annotated RC.2、8/2/1 根布局、checksums/SBOM/licenses/source identity、自动隔离/offline vertical slices 与真实 root-launcher 桌面启动均已完成；RC.2 用户结论为 `changes-required`。
7. RC.3：Assessment 技术执行、图标、全局删除与首轮拖动、候选文档、截图、annotated tag、ZIP 与仓库外 gate 均已完成；该历史候选保持不可变，用户验收结论为 `changes-required`。
8. RC.4：D-088 主动“保存全部”、D-089 节点/Raw Input Family 拖拽持久化和 D-091 工具栏本地化提示/按钮收口已在 `main` 完成代码、聚焦回归与可见预览验证；D-090 分配的新候选身份、annotated tag、唯一 ZIP/hash、隐私扫描、根布局与仓库外真实启动均已通过轻量工程门，`user_acceptance=pending`。
...
- [RC.4 Active Save and Persistent Drag Amendment](specs/2026-07-22-rc4-save-and-drag-acceptance-amendment.md)、[Plan](plans/2026-07-22-rc4-save-and-drag-implementation-plan.md) 与 [RC.3 User Acceptance Result](reviews/2026-07-22-v0.1.0-rc.3-user-acceptance-result.md)（当前主动保存、拖拽持久化、轻量候选与验收身份）
- [RC.3 Run and Model Studio Amendment](specs/2026-07-21-rc3-run-and-model-studio-acceptance-amendment.md)、[Plan](plans/2026-07-21-rc3-run-and-model-studio-implementation-plan.md)、[Verification](reviews/2026-07-21-v0.1.0-rc.3-verification.md) 与 [RC.2 User Acceptance Result](reviews/2026-07-21-v0.1.0-rc.2-user-acceptance-result.md)（Assessment、图标、删除与首轮拖动返修及历史发布证据）
- [RC.2 Portable Root Layout Amendment](specs/2026-07-21-rc2-portable-root-layout-amendment.md)、[Plan](plans/2026-07-21-rc2-portable-root-layout-implementation-plan.md)、[RC.1 User Acceptance Result](reviews/2026-07-21-v0.1.0-rc.1-user-acceptance-result.md) 与 [RC.2 Verification](reviews/2026-07-21-rc2-portable-root-layout-verification.md)（保留的根布局、唯一启动器与历史 fresh evidence）
