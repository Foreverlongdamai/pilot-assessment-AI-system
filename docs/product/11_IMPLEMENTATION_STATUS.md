...
| M4 当前状态 | Replacement Task 0–25 已完成。`e41d0e6` 新增 O12 Envelope-drift Latency，复用共享 `events` primitive 的 causal run 与 gap-aware trailing median；M4-D stage gate 已关闭。Task 25 focused/受控相关 gates 分别为 `21 passed`、`269 passed`；registry verify、Ruff/format（157 files）、ty/diff 均通过。Task 20 的 `1275 passed, 3 skipped` 及 build/isolated-wheel 仍是最新 full stage-gate 证据，本单 anchor 未重复重门。按用户节省额度的明确要求全程 INLINE，未走独立审查子代理。packaged registry 现有且仅有 O1–O12，capability count 为 12/18，preprocessor count 为 1（`movement-events-v1`）；registry fingerprint 为 `02ed9e1882e04d71b384b55366ee09080ec785d04883039e2ded7b7d2d7c68c6`，O10/O11 digests 保持 `b86a83189cb8ae5e7da965cbab0293ad928a0cefb9155f4c95594e3226854d6f`/`8618bb2d9e41e1d346f80abd4ce8d0de2fff57260029005fb7acd776c8a394c6`，O12 digest 为 `f4136c632dd2f303d9636a0ad67f0c597217cd96bf44d621a51f96f134aaf5b7`。因此准确口径是 **M4-B framework engineering-verified；M4-C/M4-D software-verified；18/18 specified、12/18 production plugins；M4 整体尚未 engineering verified；`formal_run_authorized=false`** |
| 下一里程碑 | 执行 replacement Task 26，实现并验证 H1 AOI Dwell 与 `gaze-aoi-intervals-v1` provider，启动 M4-E |
| 软件状态 | `in_progress`（M1/M2/M3 engineering verified；M4 Tasks 0–25 complete、M4-C/M4-D closed、O1–O12 与 `movement-events-v1` provider available、Task 26 next；完整 Assessment Core alpha 与 Gate B 尚未完成） |
...
- M4：O1–O12 已实现；其余 6 个 production AnchorPlugin、plugin-specific window materialization 与 raw availability 仍待实现；`AnchorResultV2`、catalog/dependency/execution-plan/immutable-request、measurement/artifact/inventory/report contracts、v0.2/M4 schema export、packaged exact-18 catalog/24 parameter resources、canonical fingerprints/runtime identity、trusted registry closure、通用 temporal/event primitives、transactional artifact sink、central scorer、typed DAG、preprocessing resolver、evaluator、public API、O1–O12 shared kernels/plugins 与 `movement-events-v1` provider 已实现，当前为 18/18 specified、12/18 production plugins 已实现；
- M4 replacement plan：Task 0–25 已完成；M4-C/M4-D 已关闭。O1 由 `b1d1fc9` 实现，O2 由 `b1a8743` 实现，O3 由 `f7d5261` 实现，O4 由 `056d9d5` 实现，O5 与 `movement-events-v1` provider 由 `1a119af` 实现，O6 由 `2ca3540` 实现，O7 由 `eb9cca6` 实现，O8 由 `15da2ea` 实现，O9 由 `db2b5da` 实现，O10 与共享 causal boolean primitive 由 `87aed10` 实现，O11 与共享 trailing causal median 由 `8672c74` 实现，O12 由 `e41d0e6` 实现并关闭 M4-D；下一步执行 Task 26 H1；不得把 M4-D 分段完成误计为 M4 整体 engineering verification；
...
下一步是依据已批准的 replacement implementation plan 执行 Task 26：实现并验证 H1 AOI Dwell 与 `gaze-aoi-intervals-v1` provider，启动 M4-E。Tasks 8–13 已依次关闭 canonical identity、trusted packaged registry/implementation closure、segment-aware temporal primitives、transactional artifact publication、central scoring，以及 typed DAG/preprocessing/evaluator/public API；Task 14–25 已完成 O1–O12、共享 event primitive 与首个 preprocessing provider，M4-B framework 与 M4-C/M4-D stage gates 已关闭。Production AnchorPlugin 当前为 12/18，M4 整体尚未 engineering verified，更不能提前进入 BN、runner 或 WinUI。Replacement plan 后续覆盖：
...
原逐任务实施计划已被取代、仅供历史追溯；replacement plan 与 Task 3/7/8 amendments 已于 2026-07-13 获明确或授权默认批准，Task 0–25 已完成，M4-C/M4-D 已关闭，下一步为 Task 26：
...
- [M4 Anchor Calculation and Evidence Availability Replacement Implementation Plan](plans/2026-07-13-m4-anchor-evidence-availability-replacement-implementation-plan.md)（已批准并按方案 A 修订；Task 0–25 已完成，M4-B/M4-C/M4-D gates 已关闭，O1–O12、共享 `events` primitive 与 `movement-events-v1` provider available，下一步为 Task 26 H1）
