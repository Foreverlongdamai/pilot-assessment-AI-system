...
| M4 当前状态 | Replacement Task 0–11 已分别由 `bc544bf`、`f56365c`、`928e9a4`、`e054620`、`1528d09`、`b63d38b`、`93c4ddb`、`583a1e7`、`3e1a006`、`cac645f`、`6db08c9`、`da8cb42` 完成；Task 11 transactional content-addressed artifact sink 已落地：per-anchor staging/commit/abort、evaluation-local read-after-commit、preprocessing identity、logical table/raw blob hash 与全局原子 abort 均有 focused 证明。Task 11 inline gate 为 `18 passed`，加既有 artifact-contract compatibility 共 `49 passed`，Ruff/format/ty clean；为节省额度未重复跑全仓、未走独立审查子代理，最近一次全仓证据仍为 Task 9 的 `1062 passed, 3 skipped`。`registry-v1.json` 仍为空，18 个 reference capability 全部 `not_implemented`，provider entry 不计入 18-anchor 基数；O1–O13、H1–H5 共 18/18 specified、0/18 production plugins；M4 尚未 engineering verified，`formal_run_authorized=false` |
| 下一里程碑 | 执行 replacement Task 12，实现 central scoring and breakdown aggregation |
| 软件状态 | `in_progress`（M1/M2/M3 engineering verified；M4 Tasks 0–11 complete、Task 12 next；完整 Assessment Core alpha 与 Gate B 尚未完成） |
...
- M4：18 个 AnchorPlugin、plugin-specific window materialization、evidence scorer、raw availability 与 O8/O13 派生证据；`AnchorResultV2`、catalog/dependency/execution-plan/immutable-request、measurement/artifact/inventory/report contracts、v0.2/M4 schema export、packaged exact-18 catalog/24 parameter resources、canonical fingerprints/runtime identity、trusted registry closure、通用 temporal primitives 与 transactional artifact sink 已实现，但当前仍为 18/18 specified、0/18 production plugins 已实现；
- M4 replacement plan：Task 0–11 已分别由 `bc544bf`、`f56365c`、`928e9a4`、`e054620`、`1528d09`、`b63d38b`、`93c4ddb`、`583a1e7`、`3e1a006`、`cac645f`、`6db08c9`、`da8cb42` 完成；下一步执行 Task 12 central scoring，不得把 framework primitive 完成误计为 production AnchorPlugin 或 M4 engineering verification；
...
下一步是依据已批准的 replacement implementation plan 执行 Task 12：实现 central scoring and breakdown aggregation。Tasks 8–11 已依次关闭 canonical identity、trusted packaged registry/implementation closure、segment-aware temporal primitives 和 transactional artifact publication；18 个 production AnchorPlugin 仍为 0/18。M4-B 尚未关闭，更不能提前进入 BN、runner 或 WinUI。Replacement plan 后续覆盖：
...
原逐任务实施计划已被取代、仅供历史追溯；replacement plan 与 Task 3/7/8 amendments 已于 2026-07-13 获明确或授权默认批准，Task 0–11 已完成，下一步为 Task 12：
...
- [M4 Anchor Calculation and Evidence Availability Replacement Implementation Plan](plans/2026-07-13-m4-anchor-evidence-availability-replacement-implementation-plan.md)（已批准并按方案 A 修订；Task 0–11 已完成，下一步为 Task 12 central scoring）
