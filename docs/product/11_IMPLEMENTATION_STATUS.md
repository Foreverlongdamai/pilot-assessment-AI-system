...
| 下一里程碑 | 创建干净 annotated `v0.1.0-rc.1` tag，构建并在仓库外验证候选；随后由用户独立验收完整候选 |
| 软件状态 | `in_progress`（M1–M8D、D-055、M8C-1 与 M8E source implementation complete；final candidate verification pending；`user_acceptance=pending`；starter/synthetic `formal_run_authorized=false`） |
...
M1/M2/M3、M4R、M5、M6、M7、M8A、M8B、M8C-1、M8D、D-055 与 M8E source implementation 已实现并关闭各自当前 engineering gate；最终 tagged candidate verification 和用户手工验收尚未完成，并可能产生返修。系统当前工程实现可以：
...
- M7 是专家工作区工程完成；其 bundled portable runtime 与 editable-source handoff 已由 M8A/M8B 提供，current-system packaging/project portability/compatibility Diagnostics 已由 M8D 提供，最终分类手册由 M8C-1 提供。clean-tagged candidate 的最后构建与外部验证属于 M8E Task 13；专用 backup/restore 已取消，真实专家校准与科学验证不属于软件 engineering gate。
...
M8A–M8D、D-055、M8C-1 与 M8E source implementation 已完成。D-078–D-081 把下一动作确定为在 clean tag 上生成并自动验证完整候选，再由用户独立验收；候选在实际接受前保持 `user_acceptance=pending`。后续工作流为：
...
6. M8E：source implementation 已完成；现在创建 annotated `v0.1.0-rc.1` tag，执行 checksums/SBOM/licenses/source identity、自动隔离/offline vertical slices 与候选交付；用户验收作为候选后的独立状态。
