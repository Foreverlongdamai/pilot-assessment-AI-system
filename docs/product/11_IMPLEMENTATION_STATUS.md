...
|---|---|
| 状态日期 | 2026-07-21 |
| 产品设计基线 | v0.8 portable engineering distribution + M8C documentation infrastructure（D-031–D-076） |
...
| 下一里程碑 | M8D project backup、restore、migration 与脱敏 diagnostics |
...
- M7 是专家工作区工程完成；其 bundled portable runtime 与 editable-source handoff 已由 M8A/M8B 提供。备份迁移、分类手册与 clean-machine final candidate 仍属于 M8C–M8E，真实专家校准与科学验证不属于软件 engineering gate。
...
M8A、M8B 与 M8C-0 已完成。下一开发里程碑是 M8D：围绕现有 managed project、RunSnapshot source identities/artifacts 和相对受管路径，建立 backup、restore、migration 与默认脱敏 diagnostics。M7 用户验收可并行，但必要返修与 D-055 必须在 M8E final candidate 前关闭。后续工作流为：
...
5. M8D：围绕现有 managed project 的 backup/restore/migration、RunSnapshot source identities/artifacts 和脱敏 diagnostics（**下一阶段**）；
