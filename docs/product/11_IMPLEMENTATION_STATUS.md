...
| 下一里程碑 | M8B-1 loaded-source/runtime/dependency/operator identity 与 RunSnapshot source artifact；随后是 M8B-2 新 operator 扩展闭环 |
| 软件状态 | `in_progress`（M1–M8B-0 engineering verified；M7 user acceptance pending；D-055、M8B-1/M8B-2 与 M8C–M8E 未完成；starter/synthetic `formal_run_authorized=false`） |
...
M1/M2/M3、M4R、M5、M6、M7、M8A 与 M8B-0 已实现并关闭各自 engineering gate；M7 用户手工验收尚未完成，并可能产生返修。系统当前工程实现可以：
...
上述证据关闭 M8B-0 engineering gate；不关闭 M8B-1/M8B-2、M7 用户验收、D-055、M8C–M8E 或科学校准。
...
M6 收尾另行核对并关闭：project locator 不保存绝对根路径；managed import 不保留 external 运行依赖；SQLite domain mutation 可加入同一 idempotency/audit transaction；draft graph/layout batch 只保存一次；stdout 只有 JSON-RPC；大型 payload 只返回 project-relative/artifact reference；运行总量由 frozen Evidence closure 动态计算；sidecar adapters 没有复制 Evidence/BN 算法。M8A 关闭首个便携打包边界，M8B-0 关闭 software-copy system model ownership；仍未实现的 M8B-1/M8B-2、M8C–M8E 与科学校准保留在 §5，不以工程测试冒充完成。

## 5. 尚未实现

- M8B-1：在 M8B-0 system ownership 基础上，补全 loaded source/runtime/dependency/operator identity、disk drift 可见性与 RunSnapshot source snapshot artifact；
...
M8A 与 M8B-0 已完成。下一开发里程碑是 M8B-1：围绕已稳定的 software-copy `system/` 与 release layout，正式冻结 loaded source identity、修改可见性和 RunSnapshot source snapshot；之后 M8B-2 关闭依赖/新增 operator 开发工作流。M7 用户验收可与 M8B/M8C-0 工程工作并行，但必要返修与 D-055 必须在 M8E final candidate 前关闭。后续工作流为：
...
3. M8B-1/M8B-2：发布目录中唯一活动、完整暴露、可直接编辑并全局生效的 Python backend source tree之 loaded identity/snapshot、新 operator 闭环和详细修改手册；
