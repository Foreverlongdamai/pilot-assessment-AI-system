...
M8A–M8E、D-055 与 M8C-1 已完成工程实现和候选自动验证。D-078–D-081 把下一动作确定为由用户独立验收完整候选；候选在实际接受前保持 `user_acceptance=pending`。阶段结果为：
...
6. M8E：annotated `v0.1.0-rc.1`、checksums/SBOM/licenses/source identity、自动隔离/offline vertical slices 与候选交付均已完成；用户验收作为候选后的独立状态。
