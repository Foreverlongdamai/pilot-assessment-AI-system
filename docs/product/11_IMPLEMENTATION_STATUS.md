...
| 软件状态 | `in_progress`（M1–M8B engineering verified；M7 user acceptance pending；D-055 与 M8C–M8E 未完成；starter/synthetic `formal_run_authorized=false`） |
...
M8A 与 M8B 已完成。下一开发里程碑是 M8C：围绕稳定的 portable layout、system model ownership 和 editable-source contract，建立 Markdown 权威原稿、自动 DOCX、术语/交叉引用检查与分册体系。M7 用户验收可与 M8C-0 工程工作并行，但必要返修与 D-055 必须在 M8E final candidate 前关闭。后续工作流为：
...
4. M8C：以 Markdown 为权威原稿、自动生成版本化 DOCX 的十二类文档和技术总册（**正在实施**）；
