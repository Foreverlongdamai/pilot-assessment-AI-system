...
4. 原始数据未声明单位时保持单位未声明；不要求用户补填、不猜测、不换算，也不阻止固定 Evidence 方法读取原始数值。
...
4. 生成 modality、字段、时间戳和 annotation 的提议映射，并只保留源文件/profile 已明确声明的单位；
...
每个 `field_mapping` 包含 raw path、raw field、canonical field、modality、physical dtype、可选
declared unit、unit provenance、timestamp role 和 resolution status。没有单位时 declared unit 为 null，
不是 unresolved input。技术 ID 默认只在“高级详情”展示；普通界面使用简短语义名称。
...
- 文件或 profile 已明确单位时，manifest 原样记录声明及 provenance；
- 文件与 profile 均未声明单位时，不要求用户输入，manifest 的 stream `units` 使用空 object，
  column metadata 不增加单位条目；
- 缺失单位不是 unresolved input，不改变 `can_materialize`，也不产生导入错误；
- adapter 不依据数值范围猜单位，不做隐式单位换算，原始数值按固定 mapping 送入现有 Evidence recipe；
- Evidence/operator 继续按其当前固定实现解释数值。这只代表 starter 计算模板可运行，不证明单位
  语义或科学阈值正确；
- runtime trace 和 Session provenance 记录 `unit_handling=undeclared-pass-through-v1`，使结果使用者
  能看到该事实，但普通导入界面不弹出单位填写表单；
- 时间戳也不要求用户输入单位。已匹配的 adapter profile 使用其固定 timestamp parsing/mapping
  规则；若 source 未声明时间单位，则 provenance 记录 fixed adapter assumption，且不把它伪装成
  source-declared unit。

可信解析代码、schema 和固定单位/时间解释属于系统级、带版本的 adapter profile；用户确认的纯
文件/字段 mapping 属于当前受管项目的 `session_import_profiles` preset，不包含或执行 Python 代码。
M7 v0.4 支持同一项目内复用；跨项目导入/导出 preset 留给 M8 交付工具，不影响每个
SessionRevision 已冻结的 exact mapping。
...
- task 所需但 profile 无法固定的 master clock 或跨设备 clock mapping（如适用）；
- 坐标系、轴方向或 control convention（仅在当前 adapter 没有固定 mapping 且该字段无法安全路由时）；
...
staging/imports/<transaction-id>/bundle/
  streams/                   # 原始 bytes 与原相对路径；兼容现有 adapter profiles
  annotations/
    ...                      # 原始 annotation bytes 与原相对路径
  _pilot_assessment/         # raw source 禁止占用的系统保留命名空间
    annotations/
...
manifest stream paths 保持 `streams/...`，因此现有 exact-path/prefix adapter profiles 不需要改写，
大型视觉数据也只复制一次。原始 `annotations/...` 保持不变，manifest annotation paths 指向
`_pilot_assessment/annotations/...` 的 deterministic canonical representation；空 canonical 文档必须
带有“source category absent” provenance，而不是伪造 phase、event 或 baseline。raw source 若已包含
`manifest.json` 或 `_pilot_assessment/`，必须按 source-kind/collision 规则拒绝，不能覆盖用户文件。
...
`present` stream 必须有路径、checksum、schema、`units` 字段和 clock mapping；`units` 可以为空
object，表示 source/profile 未声明且数值按固定 adapter pass-through。未采集 stream 使用
...
2. 验证全部必需 user resolutions 与 profile version；缺失单位不属于 resolution；
3. byte-preserving 复制原始 `streams/` 与 `annotations/` 到 staging 的同名路径；
...
4. 单位只读显示；未声明时显示本地化“未声明”，不提供填写控件，也不阻止导入；
5. 任务/文件路由等真正 unresolved items 集中显示，完成一项立即由 backend 重新验证 draft；
...
影响界面标签、帮助和错误提示；canonical 字段和技术内容保持统一英文符号。
...
| `RAW_REQUIRED_INPUT_UNRESOLVED` | 任务/字段路由/必要时钟映射等必需输入尚未完成；缺失单位不触发 |
...
2. 未声明单位不进入 `required_user_inputs`，不阻止 import，manifest units 精确为空 object；
3. 已声明单位原样保存；未声明数值原样进入固定 Evidence recipe，trace 标记 pass-through；
...
2. ViewModel 对未声明单位显示只读状态且保持 Import 可用；真正 unresolved task/routing 才禁用 Import；
3. 用户取消 picker/inspect/import 时不改变项目；
4. 中英文切换覆盖新增标签与 typed errors；
5. x64 Debug build 通过；
6. 启动真实 WinUI，确认目录选择、raw preview、未声明单位只读展示和 managed import 页面可操作。

### 12.3 产品验收

用户拿到只有 `streams/` 和 `annotations/` 的模拟器导出目录时，不需要理解或手写 manifest；在
界面补齐系统无法可靠知道且技术上必需的任务/路由元数据后，可以导入受管项目；缺失单位无需
补填。系统生成的 manifest、checksum
...
- 在 `DECISIONS.md` 写入 D-060（raw export 在受管 staging 中 materialize canonical Bundle）和
  D-061（未声明单位不猜测、不补填、不换算，以固定 adapter/Evidence pass-through 运行并留 provenance）；
