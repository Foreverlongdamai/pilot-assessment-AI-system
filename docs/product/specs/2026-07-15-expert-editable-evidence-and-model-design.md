...
专家在 BN 图中点击 evidence node 时，可以进入其 Evidence Computation Graph；从计算图也可以回到其 BN consumers。新增 evidence 可以在一个用户意图中同时创建 recipe、Anchor output、BN node 和 binding。两张图共同属于同一个 model draft 和 applied revision，但布局版本可以独立更新。
...
8. recipe 版本变化自动改变 model revision identity，不要求专家理解 hash 或 semantic versioning。
...
普通专家修改不需要 Python plugin。以下操作只修改 recipe/model revision：
...
1. 打开一个 applied revision 或 starter template；
...
7. 通过后创建 immutable applied revision，并使后续新 run 默认使用它；
...
交付 model workspace、autosaved draft、linked Evidence/BN graphs、AnchorBinding、BN node/edge/state/CPT 编辑、inference、technical validation、applied revision 和 diff/undo/replay。Starter BN 只是可编辑模板。
