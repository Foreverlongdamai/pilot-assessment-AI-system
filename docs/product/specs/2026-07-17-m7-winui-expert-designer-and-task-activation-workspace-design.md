...
> **2026-07-21 M8B-0 ownership 修订：** 本规格中的完整节点、任务激活、复制/停用、多浮窗、staged edit、Save/Discard、Ctrl+Z/Ctrl+Y 和 automatic RunSnapshot 交互继续有效；但“项目内全局节点库”、project-scoped edit session 与 `(project_id, scheme_id, node_id)` ownership 已由 M8B-0 取代。current ModelNode、TaskScheme 与 edit session 现在由每套软件副本的 `system/` 拥有，Model Studio 无需先打开 project；project 只保存 Session、运行快照、结果与 artifacts。详见 [M8B System-Owned Model Library and Editable Backend Provenance Design](./2026-07-21-m8b-system-owned-model-library-and-editable-backend-provenance-design.md)。
...
> 每套软件副本在 `system/` 中保存一个由 Raw Input、Evidence 和 BN Node 组成的全局节点库；每个任务方案只决定其中哪些完整节点处于激活状态；专家在前端复制、创建或修改节点时直接修改 system-owned canonical computation definitions；每次运行把当时的完整网络与参数冻结到目标 project，因此后续编辑不会改变历史结果。
...
2. **Session Import**：统一选择 session 数据目录，自动识别 canonical Bundle 或只有 `streams/`/`annotations/` 的 simulator raw source；只读展示 profile、映射、单位声明和缺失模态，再复制／物化到受管项目并显示 readiness 与 import progress。未声明单位只显示状态，不要求用户填写；
