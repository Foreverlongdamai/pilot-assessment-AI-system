...
| M4 候选边界修订 | 2026-07-13：proposed D-021 随完整 M4 规格批准后将 residual/overlap/coverage 固定为 non-gating diagnostics；不改变 M3 实现或 §17 golden |
...
- M3 仍报告原始 residual 和 association metrics 供 technical diagnostics/provenance；proposed D-021 随完整 M4 规格批准后，M4 不再用 residual、overlap、coverage 或所谓采集质量 gate 过滤表现 evidence。未来若要研究采集质量，必须在 M4 之外以新 decision/spec 建立独立语义。
...
`gap_detection_multiplier` 只决定 diagnostic gap threshold，不使 optional stream 自动 blocked。Residual、overlap 和最大时差只保留为 diagnostics，不硬编码进 M3 disposition，也不在 M4 变成表现 evidence gate。更改默认字段或语义需要新的 policy ID；每次运行在 report 中保存完整 policy 与 fingerprint。
