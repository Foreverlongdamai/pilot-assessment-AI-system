...
- `specs/2026-07-11-multimodal-synthetic-foundation-design.md` 第 355 行旧 `round(T×10^9)` 判定由本规格 §7 的 `master-clock-x-mapped-coverage-v1` 取代；当前 micro/real golden 中 X 末样本恰好等于 T，因此 §17 数字不变；未来 `T×f` 非整数时一律以本规格 §7 为准；
