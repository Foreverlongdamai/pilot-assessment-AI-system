...
| M4 边界修订 | 2026-07-13：accepted D-021 将 residual/overlap/coverage 固定为 non-gating diagnostics；不改变 M3 实现或 §17 golden |
...
- M3 仍报告原始 residual 和 association metrics 供 technical diagnostics/provenance；依据 accepted D-021，M4 不再用 residual、overlap、coverage 或所谓采集质量 gate 过滤表现 evidence。未来若要研究采集质量，必须在 M4 之外以新 decision/spec 建立独立语义。
