...
M4 将建立单独的完整 software fixture：在格式相容的 X/U 基础上补齐虚拟 I/G/EEG/ECG/pilot_camera、reference、phase/event、AOI、control calibration、baseline 和 anchor parameters，以验证 18-anchor 软件闭环。该 fixture 可以产生 D/A/U evidence，但仍必须标记 `scientific_validation_status=not_supported`。
...
### 14.2 Full workflow fixtures

1. `m4-all-desired-v0.1`：18/18 computed + Desired；
2. `m4-all-unacceptable-v0.1`：18/18 computed + Unacceptable、raw availability=1、`invalid_quality` count=0；
3. `m4-mixed-v0.1`：跨模态混合 D/A/U；
4. `m4-state-matrix-v0.1`：missing/config/not-applicable/dependency/error；
5. `m4-extension-v0.1`：新增、retire、参数修改、plugin version change 和旧 revision replay。

完整 fixture 补齐事件、reference、AOI、baseline 和多模态信号，不能使用 captured-format X 作为 commanded path。All-Desired fixture 必须刻意包含可快速正确响应的 disturbance、可快速恢复的 envelope excursion、一次短暂且正确修正的 envelope exit，以及 critical visual cue/fixation，使 O10/O11/O12/H2 都是 computed Desired 而不是 not_applicable。All-Unacceptable fixture 必须为相同观察机会提供完整但明确失败/极差的行为与生理信号，使 18 项全部 computed U。所有结果继续标记 software-test-only/not-supported。

### 14.3 Frozen full-workflow oracle

两套 full fixture 共享 `fixture_contract=m4-reference-fixture-contract-v1`、`generator_id=m4-multimodal-oracle-builder-v1`、seed `20260713`、session `[0,90 s]`、baseline `[0,30 s)`、Translation `[30,50 s)`、Deceleration `[50,70 s)`、Hover `[70,90 s)`，dense artifact 采样率固定为 X/U 100 Hz、I frame index 90 Hz、G gaze samples 120 Hz、ECG samples 500 Hz、EEG samples 256 Hz、pilot_camera frame index 30 Hz。每个 dense sample/frame artifact 的 source grid 固定为 `t_ns=round_half_even(k*10^9/f)` 并包含 90 s terminal row；分析 span仍为半开区间。Task event markers、fixation intervals 和 provided R-peak point artifact 使用各自合同声明的 exact source timestamp，不强制量化到 I/ECG dense sample grid；H2 fixation onset 仍必须由 120 Hz raw G samples 重算，而不是读取 fixation interval 结果。Task semantics 固定：disturbance/correct-response opportunity `35.000 s`、envelope exit/correction opportunity `55.000 s`、visual cue `60.000 s`、D-to-H boundary `70.000 s`。All-Desired 的正确 response/correction/fixation onset 分别为 35.250/55.150/60.250 s，三者分别落在 100/100/120 Hz grid；desired recovery hold 从 37.000 s 开始。All-Unacceptable 提供相同 observation opportunities，但产生明确错误方向或 miss。

Fixture builder 与 expected-oracle evaluator 必须是两个不 import `pilot_assessment.anchors` 的独立 test utilities；builder 只能使用下列冻结输入 recipe，oracle evaluator 只使用公开公式/常量重新计算 expected raw values，不能把 expected AnchorResult、production plugin 或 production artifact 回灌成输入。M4 实施计划的第一项 fixture RED gate 必须先生成并 code-review `fixture-recipe-v1.json`、bundle manifest 和 source hashes，**再**编写任何 production AnchorPlugin；recipe 值或下列 oracle 不一致时必须修改/重新批准本规格，不能调 fixture 迎合实现。

冻结输入 recipe：

| 输入族 | All-Desired | All-Unacceptable |
|---|---|---|
| X/reference/envelope | Independent commanded path；joint desired mask 在 Translation 仅 `[35,37)` 为 false、Deceleration 仅 `[55,55.300)` 为 false、Hover 全 true，故 phase 值为 90/98.5/100%；这两个 false span 由独立姿态/速率 envelope dimension 驱动，不把 position error 推过 1 ft；tracking error peak 1 ft；D→H 后 arrival overshoot 1 ft、72 s 起满足 hover envelope，stable mask 精确 `[72,84)` | joint desired mask 在 Translation 的 `[35,50)`、Deceleration 的 `[55,70)` 和整个 Hover 为 false，故 phase 值为 25/25/0%；tracking error peak 8 ft；始终不满足 capture/recovery/stable-hover 条件；O12 exit run 从 55 s 开始 |
| U/calibration | 四个角色映射到独立 control columns：workload/reversal axis、magnitude axis、event-response axis、hover-trim axis。每个 20 s phase 的 workload axis 在 `phase_start+0.5+0.95*k, k=0..20` 形成交替 `+10%/-10%` full-travel turning points；`W_min=1 Hz`，O5/O7 只声明该 axis。Magnitude axis 恒为 `+20%`。Event axis 在 35.250/55.150 s 沿 mapped correct sign step 10% 并保持 150 ms。Hover-trim axis 在 72.5、74.25、76、77.75、79.5、81.25、83 s 形成交替 `+1%/-1%` turning points | workload axis 每 phase 在 `phase_start+0.1+0.165*k, k=0..120` 形成交替 `+10%/-10%` turning points，得到 120 movements/20 s；magnitude axis 恒为 `+70%`；event axis 在 35.100 s 先沿错误方向 step 10% 并保持 150 ms，55 s 后不产生 qualifying correction；无 stable span使 O9 走 override |
| I/G/AOI | 全视野 taxonomy 固定为 `primary/secondary/other_scene`；所有 applicable gaze support 中 98% label 为 primary、2% 为 other_scene；60.250 s 起在 relevant AOI 形成至少 100 ms fixation | gaze support 完整但 100% label 为 other_scene；visual cue 后无 relevant-AOI fixation |
| ECG/R-peaks | raw ECG 只作格式相容信号；packaged input 使用 provided R-peaks。Baseline second-peak RR 精确 1,000,000,000 ns；task RR 精确 952,380,952 ns，使 signed HR delta 在 tolerance 内为 +5% | baseline 相同；task RR 精确 666,666,667 ns，使 signed HR delta 在 tolerance 内为 +50% |
| EEG | 4 个 configured channels 的 phase offsets 为 `0,pi/2,pi,3pi/2`，使 common average 为 0；每个 baseline/phase processing segment 使用 `sin(2*pi*6*t+phi)+sin(2*pi*10*t+phi)+A_beta*sin(2*pi*20*t+phi)`，baseline `A_beta=1`，task `A_beta=sqrt(1.10)` | 同一 grid/phase/channel recipe，task `A_beta=sqrt(1.75)` |
| pilot_camera | 30 Hz deterministic RGB8 frames/index；当前 18 anchors 不消费，但 checksum/provenance 必须存在 | 同左 |

Piecewise X/U 的 mask/event 边界落在固定 grid row；turning-point signal 在相邻列出的 turning timestamps 之间做唯一线性插值，在首个/末个 turning point 外分别保持首值/末值，event step 从 onset grid row 起保持规定的 150 ms 后回到 baseline。精确 sample arrays、semantic envelope/control mappings 和所有 unit conversion 常量由 `fixture-recipe-v1.json` 机械展开，禁止随机 jitter 或为通过 production plugin 反向调参。首次生成的 recipe、bundle manifest、每个 source file 和 expected-oracle file SHA-256 必须写回 fixture manifest 并在后续 revision 保持不可变；任何 generator/recipe 变化发布新 fixture version/hash，不能静默改 golden。

| Anchor | All-Desired expected raw/override | State | All-Unacceptable expected raw/override | State |
|---|---|---|---|---|
| O1 | phase 90/98.5/100%，session `90%` | D | `0%` | U |
| O2 | `1.0 ft` | D | `8.0 ft` | U |
| O3 | overshoot `1.0 ft`, settling `2.0 s`, composite primary null | D | `capture_missed`, primary null | U |
| O4 | `12.0 s` | D | `0.0 s` | U |
| O5 | `W/W_min=1.0` | D | `W/W_min=6.0` | U |
| O6 | `20.0%` | D | `70.0%` | U |
| O7 | `1.0 Hz` | D | `6.0 Hz` | U |
| O8 | `0.81` | D | `0.0` | U |
| O9 | `0.5 Hz` | D | `no_stable_hover`, primary null | U |
| O10 | `2.0 s` | D | `recovery_missed`, primary null | U |
| O11 | `250 ms` | D | wrong-direction `response_missed`, primary null | U |
| O12 | `150 ms` | D | `correction_missed`, primary null | U |
| O13 | signed HR delta `+5%`, coupling loss `0%` | D | signed HR delta `+50%`, Q_control `0`, coupling loss `100%` | U |
| H1 | on-task dwell `98%` | D | all gaze `other_scene`, on-task `0%` | U |
| H2 | `250 ms` | D | `fixation_missed`, primary null | U |
| H3 | off-task dwell `2%` | D | off-task dwell `100%` | U |
| H4 | absolute HR delta `5%` | D | absolute HR delta `50%` | U |
| H5 | absolute engagement delta `10%` | D | absolute engagement delta `75%` | U |

所有 t_ns、count、枚举、override payload 和 one-hot likelihood 必须 exact。非 DSP algebra golden 使用 absolute tolerance `1e-9`；Butterworth/Welch/PSD/HR-derived float 使用 `abs<=1e-6 OR rel<=1e-6`，但 D/A/U boundary test 必须使用构造后明确位于边界两侧且超出 tolerance 的值。两套 fixture 都要求 18 executed/18 computed；All-Unacceptable 的 raw availability=1、M5 无 user exclusion 时 model availability coverage=1。

### 14.4 Completion commands

实施计划必须给出并实测：

- focused contract/plugin/anchor tests；
- full pytest suite；
- JSON Schema regeneration 与 symmetry checks；
- Ruff/formatter；
- strict type check；
- build/wheel install；
- isolated-wheel M1→M2→M3→M4 smoke；
- repository-external format sample interface test；
- source checksum/byte immutability；
- cross-process deterministic fingerprints。

Isolated-wheel smoke 的 public entry 固定为 `python -m pilot_assessment.verification.m4_smoke --fixture <bundle>`，内部只调用公开 `pilot_assessment.anchors.api.evaluate(request, sink)`。Wheel 必须包含 v0.2 contracts/schemas、registry resource、18 plugin factories、parameter schemas 和 smoke runner；full fixture 可以作为测试 artifact 位于 wheel 外。Smoke 在 repository 外临时 cwd、清空 project-root `PYTHONPATH` 后运行，并断言 imported module path 位于安装环境而非源码树。

## 15. M4-A 至 M4-G 完成门

| 阶段 | 范围 | 可声称状态 |
|---|---|---|
| M4-A | 本规格、D-021–D-025、AnchorResult v0.2、catalog/plan/report schema | 18/18 specified，0/18 implemented |
...
| M4-G | full E2E、extension、determinism、wheel、docs/handoff | M4 engineering-verified |
...
为让用户复核的是一个完整、无跨文档冲突的候选基线，下列文档已在同一个 design-only change 中按本规格作 **candidate alignment**；这些文字在完整 M4 规格获批前不把 D-021–D-025 冒充为 accepted，也不构成实现。批准后只需把决策状态与入口状态改为“已接受”，并保留单独 commit 轨迹；若用户要求改变语义，再重新执行相应迁移。
