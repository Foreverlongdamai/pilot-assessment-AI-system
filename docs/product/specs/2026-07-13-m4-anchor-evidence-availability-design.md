...
| 实现状态 | 18/18 已设计，12/18 production plugins 已实现；原 M4 实施计划已被轻量修订取代；replacement Task 0–25 已完成，M4-B framework 与 M4-C/M4-D stage gates 已关闭，O1–O12 shared kernels/plugins、共享 `events` primitive 与 `movement-events-v1` provider 已落地且 capability 均为 `available`；下一步为 Task 26 H1；M4 整体尚未 engineering verified |
