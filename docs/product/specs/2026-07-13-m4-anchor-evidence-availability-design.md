...
| 实现状态 | 18/18 已设计，13/18 production plugins 已实现；原 M4 实施计划已被轻量修订取代；replacement Task 0–26 已完成，M4-B framework 与 M4-C/M4-D stage gates 已关闭、M4-E 进行中，O1–O12/H1 plugins、共享 `events`/gaze-AOI primitives 与 `movement-events-v1`/`gaze-aoi-intervals-v1` providers 已落地且相关 capability/provider 均为 `available`；下一步为 Task 27 H2；M4 整体尚未 engineering verified |
...
Task 15 将该版本化坐标绑定收口为 execution entry 的不可变 `temporal_recipe`：exact ordered `axis_bindings`，非负 `reference_gap_threshold_ns`，以及 exact `coordinate_transform={transform_id,source_frame_id,target_frame_id,matrix_row_major[9],translation_m[3]}`。三轴合同单位先确定性转换到 metre，reference 再经显式 affine transform 进入 X frame，最终误差统一输出为 ft；缺失或 frame 不匹配时不得猜测，返回 `not_computable`。该 binding 随 plan fingerprint 冻结，属于 M5 compiled-plan/model binding，不是 O2 的可编辑表现阈值；O2 v0.1 plugin parameters 仍严格为 `{}`。
...
Task 18 的实现契约收口不改变上述公式、阈值或公开 schema：`AnchorPluginContext` 向 provider 投影其 required modalities 对应的已验证 `ResolvedInputTableContract`，且 stream projection fingerprint 绑定这些合同；`movement-events-v1` 明确选择 `U/samples`，不会在同一 U stream 有其他 table role 时按字段形状猜表。provider 仍是 session scope，并在既有 `event_kind` 字段中使用 `support-start`/`support-end` 行保存每个 continuous support segment，使 O5 能按 phase/channel 精确重建 gap-aware denominator；turning point、movement 与 short-filter-bypass 诊断也保留在 provider product。O5 对外的 `movement-events-v0.1` anchor artifact 只筛选真实 `event_kind=movement` 行，因此 provider support metadata 不冒充行为事件。
...
Task 19 的实现契约收口不改变上述公式、阈值或公开 schema：O6 只读取执行计划绑定且经 M3/M4 合同验证的 `U/samples`，按 applicable phase 的半开区间重建 native left-hold support；部分时间支持直接按实际 observed duration 计算，不设置 coverage gate。每个 phase/channel 的归一化平方积分先保留，再跨 phase 按积分之和与 support duration 之和池化，因此不会把不同持续时间的 phase 百分比简单平均。任何有限控制值都进入公式，端点外值不裁剪；只有无时间支持或合同/数学结构不完整才进入非 computed 状态。`rms-contribution-trace-v0.1` 每个 phase/channel 发布该 scope 的 RMS 与冻结权重。
...
Task 20 的实现契约不改变上述公式、阈值或公开 schema：O7 只消费注册并缓存的 `movement-events-v1` immutable product，不复制 movement detector。反转核只配对同一 support segment 内的相邻 turning points；两个 phase 首尾相接时，共享边界 timestamp 确定归属后一个 segment，禁止跨 phase 形成虚假反转。各通道先将全部 applicable phase 的 qualifying count 与 observed support duration 分别求和，再计算 rate，session 取通道最大值；任何 applicable phase 非 computed 时不发布局部 session score。零反转是有效的 `computed + 0 Hz`，有限高频反转是有效负面 evidence，不按数值大小过滤。
...
读取 computed O1/O5 数值；上游即使 U 仍继续计算。O5 的 session `primary_value` 已固定为 `r=W/W_min`，因此 production O8 使用严格等价式 `TPX=(P/100)^2/sqrt(max(r,1))`，不复制或重新维护 O5 的 `w_min_hz` 参数。D `>=0.6`，A `>=0.4`，否则 U。默认 `likelihood_strength=0.50` 由 M5 应用。
...
Task 22 将该语义收口为可执行绑定而不改公式或阈值：compiled recipe 必须显式携带 `control_channel_ids` 与来自 X mask 时间合同的 `mask_gap_threshold_ns`（允许显式 `null`，但不得省略），避免把 U gap 与 X mask gap 混为一谈。O1 mask 先按同一 native X sample 对所有 axis 做 `all(inside)`，再与 O4 mask 以 `(phase_id,t_ns,source_row_id)` exact join；缺少 mask inventory 是 plan/dependency contract error，不得伪装成 `no_stable_hover`。布尔交集采用 native left-hold 且不跨 gap；每个 qualifying leading sample 只有在 raw U 存在 deterministic nearest-within match 时才贡献时段，连续时段随后与已注册 movement product 的 channel support 做半开区间交集。O9 信任 provider 已应用其自有的 inclusive movement 下幅值阈值，只对 event timestamp 的 `[start,end)` 归属和自己的 inclusive `micro_movement_max_amplitude_pct` 上界再次筛选；默认 reference provider 的下界为 `0.5%`，因此默认有效区间仍是 `[0.5%,maximum]`，但专家修改 provider 下界时无需同步复制到 O9。各 channel 跨所有已有 matched phase 汇总 count 与 denominator 后计算 rate，session 取最大 channel。部分 phase 或 sample 未匹配不触发 coverage gate；只要任一 matched support 存在即从实际支持计算。
...
Task 24 的 production binding 使用 exact `event_ids`、`phase_bindings`、U `table_role`、timestamp/in-session/stable-row 字段与 M3-derived gap threshold；event 自身的 `control_mapping_ids` 选择 `ControlEffectMapping`，不从数据猜 channel 或 sign。每个 raw control 先按 mapping 的 lower/trim/upper 转为带符号 percent-full-travel，再在同一 continuous support segment 内执行 timestamp-based inclusive 20 ms trailing median，不跨 gap。Baseline 只取同 phase、事件前至多 1 s 的非空 filtered samples；只有该集合为空时才使用 mapping trim（归一化后为 0），不会用 pilot 后续表现反推 baseline。正确/错误方向 run 均使用严格 `>5%` 与 inclusive `>=100 ms`，later confirmation 保留 causal onset；event observation end 裁到 2 s horizon、phase、opportunity 与 session end 的最早值。跨 channel 取最早正确 run；只有 qualifying wrong run 严格早于最早正确 run（或根本无正确 run）才触发 `response_missed`，subthreshold noise 不触发 veto。所有 applicable events 都输出 breakdown/trace，成功 session 取最差 latency，任一 miss 否决；无绑定 event 为 `not_applicable`、缺 event mapping 为 `not_computable`、真正缺 U support 为 `missing_input`。实现不设 coverage/quality filter，artifact ID 为 `response-events-v0.1`。
...
Task 25 的 production binding 使用 exact `phase_bindings`、`envelope_id`、`control_mapping_ids`，并分别绑定 X/U 的 table role、timestamp、in-session、stable-row 字段与 M3-derived gap threshold。X 以 native left-hold support 检测 desired-envelope 外连续 run；`>=100 ms` 只负责确认，事件 `exit_t_ns` 保留首个越界时刻。每个在 `t_exit` 越界的 axis 使用当时 signed state error；`ControlEffectMapping.correct_sign` 定义正 state error 的正确 control 方向，实际方向固定为 `correct_sign * sign(state_error)`，不从 pilot 后续动作反推。U 先按 lower/trim/upper 转成 signed percent-full-travel，再在 continuous support segment 内执行 inclusive 20 ms trailing causal median；baseline 只取同 phase、越界前至多 1 s 的非空 filtered samples，完全没有前置样本时才回退 mapping trim。正确/错误方向均使用严格 `>5%` 与 inclusive `>=100 ms`，跨 mapped channel 取最早正确 onset；只有 qualifying wrong onset 严格早于正确 onset（或无正确 onset）才触发 `correction_missed`，subthreshold noise 不否决。观察终点裁到 2 s horizon、phase 与 session end 的最早值；所有已确认 exit 都保留 breakdown/trace，成功 session 取最差 latency，任一 miss 否决。未形成确认 exit 为 `not_applicable`，缺 state-axis/effect mapping 为 `not_computable`，真正缺 X opportunity support 或已确认 exit 后缺 U support 为 `missing_input`。实现不设 coverage/quality filter，artifact ID 为 `correction-events-v0.1`。
