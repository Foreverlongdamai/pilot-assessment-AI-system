...
| 当前执行入口 | [M8E Final Release Candidate Design](2026-07-21-m8e-final-release-candidate-and-handoff-design.md) 与 [M8E Implementation Plan](../plans/2026-07-21-m8e-final-release-candidate-implementation-plan.md) |

> **2026-07-21 适用性修订：** 用户取消专用 backup/restore 产品。D-077 与 M8D 正式规格取代本文关于 `.paprojbackup`、Backup/Restore UI、diagnostic archive 和“backup 是 M8 完成门”的候选文字。M8D 改为显式捕获当前已保存 system、完整 project 目录复制、compatibility 与轻量 diagnostics。

> **2026-07-21 M8E 门修订：** D-078–D-081 取代本文把 M7 用户验收作为 M8C-1/M8E 构建前 Gate 0 的口径。当前流程为 `M7 engineering verified -> D-055/M8C-1/M8E -> v0.1.0-rc.1 -> 用户验收`；候选可先构建，但在用户实际接受前始终标记 `user_acceptance=pending`，不得称为正式 `v0.1.0`。
...
M7 用户验收仍可能发现窗口布局、操作路径、字段暴露方式、错误恢复和帮助入口问题。D-078 允许先完成可整体检验的候选，并遵循以下规则：

1. M7 现有代码和工程测试结果保持不变；
2. M7 状态写为“engineering verified，user acceptance pending”；
3. 用户已明确授权完成 D-055、M8C-1 与 M8E，再直接验收完整候选；
4. 候选截图可以标记为 `release-candidate`，但不能在用户验收前标记为 final；
5. M8A、M8B、M8C-0 与 M8D 已由各自正式规格/计划收口；M8E 统一完成剩余代码迁移、M8C-1、候选打包与自动隔离验证。
...
8. 软件关闭后，完整 project 目录可复制到另一位置或 Windows 设备并重新打开；正式发布包携带明确选定的 current system；
...
## 4. 已取代的 Gate 0 与最终候选验收

旧路线将 M7 用户验收设为 **M8E 最终交付候选** 的构建前硬门。D-078 已将其取代：M7 engineering verification 后先完成 D-055、M8C-1/M8E 和 `v0.1.0-rc.1`，再由用户直接验收完整候选。工程自动化仍不能替代用户验收；它只允许候选在 `user_acceptance=pending` 状态下形成和交付。

### 4.1 完整候选的用户验收应覆盖的真实操作
...
| 仅与便携运行、源码交付/修改、current-system packaging、项目迁移或交付有关 | 进入 M8 正式规格 |
| 与模型科学合理性有关 | 记录为专家校准事项，不阻塞 M8 软件产品化 |
| 新的产品能力或架构方向 | 单独讨论，不自动扩大 M8 |

### 4.3 最终用户验收关闭条件

M8E 候选不以本节为构建前置。只有同时满足以下条件，候选才可从 `user_acceptance=pending` 进入 accepted，并据此重建正式版本：

- 用户明确表示 M7 手工验收完成；
- 已确认的 M7 缺陷和必要返修已经关闭；
- M7 关键界面、字段、协议和项目格式不再处于本轮变动中；
- 用户验收针对完整、可追溯的 release candidate，而不是中间 development build；
- 验收记录明确对应 candidate label、source commit、system identity 和 documentation identity。
...
精确的 .NET SDK、Windows App SDK、Python、第三方 wheel 和文档工具版本由后续正式 M8A/M8C/M8E 规格与 release toolchain lock 冻结；不再等待已由 D-078 取代的旧 Gate 0。
...
| 10 | 系统分发、项目迁移与故障排查 | 用户、支持、维护者 | how-to | 如何发布 current system、复制整个软件/project 目录、判断兼容性和处理常见故障 |
...
## 10. Current-system 分发、项目迁移与诊断

### 10.1 不改变 ownership 原则

现有受管 project 已把导入 session 复制到项目目录，并用 SQLite、artifacts 和 immutable RunSnapshot 保持可移植性。Current Evidence/BN/CPT/task/layout 属于软件副本的 `system/`；Python 实现属于 `backend/src/`。M8 不引入第二套数据模型或 backup archive。

### 10.2 发布 current system

正式 builder 必须通过显式 `--system-source` 捕获专家已经保存并关闭的 current system：

- 要求 app 已关闭、writer lock 可取得、canonical/edit workspace clean；
- 使用一致 SQLite capture，不在输入无效时重新 seed starter model；
- 动态记录 model identity 与 node/scheme counts，不固定 starter 数量；
- 拒绝包含 project/session/run/result/artifact user-owned rows 的 system；
- package 不含任何用户 project 或测试 Session。

### 10.3 Project 复制与 diagnostics

项目迁移要求软件关闭后复制完整 project 根目录，再通过“打开项目”使用现有 integrity/schema/recovery checks。M8D 不提供 Backup/Restore UI、archive、merge 或云同步。

现有 Diagnostics 技术区域至少说明：

- 产品、runtime、protocol 与 schema 版本；
- current system model identity 与 node/scheme counts；
- loaded Python source、baseline divergence、operator catalog 与 restart requirement；
- 当前 project compatibility/recovery state 与最近 sidecar error。

M8D 不生成 support archive。任何未来 diagnostics export 默认不得包含原始 X/U/I/G/P、pilot-camera、受试者标识、完整结果内容或用户私有路径。
...
| M8C Documentation and DOCX Pipeline | 建立分类 Markdown 原稿和版本化 DOCX 生成系统 | 12 类手册、C4/Diátaxis 结构、截图/图表、总册、文档校验 | M8E 候选合同 stable；candidate screenshots 由 D-080 管理 |
| M8D Current-System Packaging, Project Portability and Diagnostics | 发布已保存的 current system，并验证完整 project 目录复制与轻量诊断 | explicit system source、dynamic model baseline、project move/reopen、diagnostics parity | M8A/M8B storage and ownership stable |
| M8E Release Candidate and Handoff | 汇总并完成如实声明的自动隔离验证与候选交付 | ZIP、checksums、SBOM、licenses、manuals、engineering evidence、pending acceptance record | M8A–M8D complete |

默认执行为 INLINE、按小型垂直切片推进，不进行大规模 subagent fan-out。M8 正式实施仍采用 contract-first 和选择性轻量测试；只有 release/current-system capture/source identity 等高风险不变量需要自动化门，不为 starter 科学内容建立重型 golden，也不为每次专家源码保存设置审批门。

## 13. 路线图决策状态

| 候选 ID | 候选口径 |
|---|---|
| C-M8-01 | **由 D-078 取代：** M7 engineering verified 后可先构建完整 M8E 候选；用户验收仍不可由工程验证替代，但改在完整候选交付后执行 |
...
| C-M8-07 | **由 D-077 取代：** 不建立 backup/restore 产品；正式 builder 显式捕获 current system，project 在应用关闭后通过完整目录复制移动，RunSnapshot identity 继续保留 |
| C-M8-08 | **由 D-077 修订：** M8 工程完成门是 clean-machine portable workflow + current-system capture + direct Python source modification + project copy/reopen + docs 验收；科学校准继续独立记录 |

C-M8-01 已由 D-078 取代；候选身份、候选截图和两层验收证据分别由 D-079–D-081 冻结。C-M8-02–C-M8-08 已由 D-062–D-077 及对应正式 M8 子规格收口。

## 14. M8 完成定义候选

M8E release candidate 只有在以下工程条件满足时才可称为“engineering candidate verified”；最后两项继续作为正式 accepted release 的独立门：

- 便携包在仓库外 disposable extraction、restricted `PATH` 且不调用系统开发工具的声明环境中解压启动；
...
- 完整 project 目录可复制到新目录并保持历史 run/replay；
- 正式 package 精确携带明确选定、已保存且已关闭的 current system，不静默回退到 starter seed；
...
- 候选交付时明确写为 `user_acceptance=pending`；
- 用户完成最终产品验收后，才可从对应 source 重建并称为 accepted `v0.1.0`。

## 15. 正式子规格冻结的问题

以下问题最初未在本候选大纲中提前定死；其中已完成项由对应正式规格取代，未执行项必须如实保持 pending：

1. 完整候选验收后需要保留或调整的具体窗口、字段和帮助入口；
...
7. current-system capture 的 source selector、locking、SQLite consistency 和 dynamic manifest contract；
8. DOCX 模板、字体、截图主题、英文交付深度和文档构建工具锁；
9. 自动隔离验证使用仓库外 disposable extraction 与 restricted `PATH`；Windows Sandbox、虚拟机或独立设备只有实际执行时才记录，由 D-081 管理；
