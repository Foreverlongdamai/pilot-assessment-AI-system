...
| 实现状态 | Replacement Task 0–23 已完成；M4-C stage gate 已关闭、M4-D 正在执行；O1–O10 capability、共享 `events` primitive 与 `movement-events-v1` provider 均为 `available`，下一步为 Task 24 O11；18/18 specified、10/18 production plugins implemented；M4 整体尚未 engineering verified |
