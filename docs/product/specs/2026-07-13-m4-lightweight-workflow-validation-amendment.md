...
| 实现状态 | Replacement Task 0–24 已完成；M4-C stage gate 已关闭、M4-D 正在执行；O1–O11 capability、共享 `events` primitive 与 `movement-events-v1` provider 均为 `available`，下一步为 Task 25 O12；18/18 specified、11/18 production plugins implemented；M4 整体尚未 engineering verified |
...
Replacement Task 0 已完成批准的安全检查：provisional heavy fixture files 已逐项移除且未进入提交历史，随后首先观察到缺少轻量能力的正确 RED，再实现新的 input-only fixture。提交 `bc544bf` 冻结了 10 秒 recipe、独立 exact-18 expected vector 与 source hashes；Task 1 `f56365c` 完成 numeric/JCS runtime 审计；Task 2 `928e9a4` 完成 breaking `AnchorResultV2`；Task 3 `e054620` 完成 session-bound candidate/三参数 binder；Task 4 `1528d09`、Task 5 `b63d38b` 与 Task 6 `93c4ddb` 完成 catalog/plan/request、measurement/report/runtime contracts 和 deterministic schema publication；Task 7 `583a1e7` 发布 exact-18 catalog、24 个 canonical parameter resources 与 zero registry。此后 Task 8–13 完成 canonical/runtime framework，Task 14 `b1d1fc9`、Task 15 `b1a8743`、Task 16 `f7d5261`、Task 17 `056d9d5`、Task 18 `1a119af`、Task 19 `2ca3540`、Task 20 `eb9cca6`、Task 21 `15da2ea`、Task 22 `db2b5da`/`a76abde`、Task 23 `87aed10` 与 Task 24 `8672c74` 分别完成 O1–O11；Task 18 同时注册首个 `movement-events-v1` preprocessing provider，Task 22 follow-up 明确 O9 信任 provider-owned movement 下幅值阈值，Task 23 新增 causal boolean interval/run `events` primitive 与 `recovery-events-v0.1`，Task 24 扩展 gap-aware trailing causal median 并发布 `response-events-v0.1`。Task 24 focused/受控相关 gate 分别为 `18 passed`、`229 passed`；最新 full-repository/build/isolated-wheel stage 证据仍为 Task 20 的 `1275 passed, 3 skipped` 及其完成门，不得把旧 provisional `8 passed` 记入 M4 完成证据。M4-C stage gate 已关闭、M4-D 正在执行；下一步为 Task 25 O12，当前为 11/18 production plugins。
