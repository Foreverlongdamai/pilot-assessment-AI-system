...
`BnNodeVersion.probabilistic_parents` 可以引用 exact BN-node versions，或在专家定义的其他 DAG 中引用 exact Evidence observation bindings。`EvidenceVersion.source_bindings` 则只能引用 raw/session/task sources，不得引用 latent BN posterior；两种 parent/dependency namespace 永不共用。
...
它回答“EvidenceRecipe 读取什么”。后端 canonical 字段为 `source_bindings` 或 recipe 内的 typed operator ports。它：
...
- X/U/I/G/P 多模态输入接口；
- O1–O13、H1–H5 的 18 个 Evidence concepts/versions；
...
M4R 当前 canonical `EvidenceRecipe`、operator registry、validator、compiler/executor、draft/preview/apply/replay 和 18 个 starter resources 是 M5 的输入，不推翻已有算子或算法代码。

迁移原则：

1. 现有 recipe identity 导入为 `EvidenceConcept + EvidenceVersion`；
2. M4R applied revision 作为 migration lineage 记录，不在原地改写；
3. 现有 18 recipes 组成 starter scheme 的 Evidence 部分；
4. M5 新增全局 component library、BN versions、Evidence binding、scheme composition 和 atomic publish；
5. 旧 whole-Anchor plugins 继续仅用于历史重放/迁移比较；
6. 不要求新版本维持 provisional starter 数值等价。
...
本文固化了用户已确认的 M5 核心架构，但在用户复核本书面版本前不编写 implementation plan、不声称 M5 代码已完成。若书面复核通过，下一步才是把本规格拆成轻量、inline 的 M5 实施计划。
