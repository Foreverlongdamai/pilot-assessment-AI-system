...
| 字段 | 值 |
|---|---|
| 设计状态 | Review candidate |
| 日期 | 2026-07-11 |
| 上位基线 | Product design v0.1 + Backend Foundation M1 |
| 本阶段 | M2：理想多模态合同、合成 bundle、全模态 ingestion preflight |
...
因此，当前 CSV 保持原样作为 X/U 共享源，其余模态以各自理想文件加入同一 bundle。这样能够真实测试多采样率、missingness、clock mapping 和文件合同，也不会形成未来必须拆除的超宽 CSV。
...
Header 含不一致的前导/尾随空格。Adapter 必须同时保留 raw header，并使用版本化的 `trim_outer_ascii_whitespace_v1` 规范化后匹配；规范化后如产生碰撞则阻止 preflight。
...
这是采用方案。X/U 共享原 CSV；I/G/EEG/ECG/pilot_camera 使用理想文件合同；生成器、adapter 和 preflight 依赖同一套版本化 schema/profile。
...
- stream 可以与 stream 共享，但 stream 不可与 annotation/integrity 共享同一 path。
...
- manifest `source_session.system = synthetic-multimodal-generator-v0.1`，并在 provenance 保留原始 X/U 文件 hash；这是 real-X/U + synthetic modalities 的混合测试 bundle；
...
### D-013：M2 只产生 ingestion-stage readiness

M2 `PreflightReport` 的 scope 是 `inspect_only_ingestion_content_v1`。它可以允许进入 M3 synchronization，但 `formal_run_authorized` 永远为 false。原始 source artifact 在 M2 可以只含 source timestamp 与声明的 `clock_sync`；`03_SESSION_BUNDLE_SPEC.md` 要求的 authoritative `t_ns` 解释为 M3 产出的 aligned view 字段，不由 M2 adapter 伪造。
...
`task_reference` 是同一 major version 下的可选扩展 stream，不属于七个 core modalities，但通过普通 adapter/preflight 进入 session context。
...
| task_reference/commanded_path | `task-reference-path-raw-v0.1` | `source_timestamp_s, reference_sample_id` | profile per state channel |
...
顶层 composite schema 分别为 `vr-scene-source-bundle-v0.1`、`gaze-source-bundle-v0.1`、`eeg-source-bundle-v0.1`、`ecg-source-bundle-v0.1`、`pilot-camera-source-bundle-v0.1` 与 `task-reference-source-bundle-v0.1`。每个 descriptor 的 artifact roles 必须与 registry 完全一致。
...
`aoi_instances.parquet` 每个 frame/AOI 一行，包含 `frame_id`、`aoi_id`、taxonomy version、normalized bbox x/y/w/h、visibility 和 confidence。PNG 使用 phase-dependent background 与稳定 AOI 色块，目的是验证 scene/gaze binding，不追求视觉真实性。
...
- commanded path 以 X 轨迹的确定性平滑版本加受控偏差生成，仅用于软件 golden tests；
...
| task_reference/commanded_path | `reference_sample_id:u64`, `source_timestamp_s:f64`, `target_x_m:f32`, `target_y_m:f32`, `target_z_m:f32`, `target_vx_m_s:f32`, `target_vy_m_s:f32`, `target_vz_m_s:f32`, `target_roll_deg:f32`, `target_pitch_deg:f32`, `target_yaw_deg:f32`, `envelope_profile_id:utf8` |
...
3. 定义 `control_activity(t)=min(1, abs(Pilot Lon raw)/100)`，该值仅驱动 synthetic generator，不是对真实控制单位的归一化结论；
...
9. generator 完成后写 manifest/checksum，并再次通过 M1 loader 与 M2 preflight；
...
contracts/ingestion.py
  PreflightReport / StreamPreflightResult / status enums
...
- 七个 core modalities 加可选 stream 的 deterministic results map；
...
- source snapshot fingerprint，用于发现 preflight 期间文件变化。

大数组不进入 report。内部 `IngestionPreflightOutcome` 组合 report 与 PreparedSession；协议层只发送 report。

新增 `preflight-report-0.1.0.schema.json`，并与 Pydantic 进行正反例对称测试。
...
- PNG adapter 禁止动画和 ancillary payload，要求 RGB8、实际尺寸与 frame index 一致；synthetic profile 只接受 64×36 或 48×48，通用安全上限为单图 16 megapixels，Pillow decompression-bomb warning/error 一律阻止 preflight；
- 所有 bytes/rows/columns/string length 使用 adapter resource limits；
- source 在 inspect 与 adapter read 之间变化则 `SOURCE_CHANGED_DURING_PREFLIGHT`。
...
### 12.2 Local real-data validation

真实 CSV 不进入 Git。测试命令显式传入源路径，在 `local_data/` 生成 full bundle，验证：
...
| 里程碑 | 交付 | 退出条件 |
|---|---|---|
| M2 | 理想多模态合同、generator、全模态 ingestion preflight | synthetic full bundle 七个 core modalities ready |
...
| M6 | pipeline/CLI/report/provenance/replay | bundle→preflight→anchors→BN→report 一键跑通 |
...
- 真实 CSV 与生成 bundle 继续位于仓库外或 `local_data/`；
...
1. D-011/D-012/D-013 写入 DECISIONS；
2. M1 全套测试保持通过；
3. shared X/U artifact 的安全与 checksum tests 通过；
4. profile-driven real CSV adapter 在本地实测得到 2,902 rows、100 Hz、X/U/context 正确映射；
...
7. `PreflightReport` Schema 完成并确定性导出；
8. full synthetic report 为 ready、可进入 M3、formal_run_authorized=false；
9. 真实 CSV 与生成 bundle 未出现在 Git tracked files；
...
- 当前真实 CSV 的路径、hash、行列数和时间统计均已复核；
