...
| Anchor 实现状态 | 18/18 已设计，10/18 production plugins 已实现；原 M4 实施计划已被取代，replacement plan 与 Task 3/7/8 amendments、D-028–D-030 已批准；Task 0–23 已完成，M4-C stage gate 已关闭、M4-D 正在执行，O1–O10 capability 均为 `available`，共享 `events` primitive 与 `movement-events-v1` provider 已实现，下一步为 Task 24 O11 |
...
当前只可声称 **18/18 anchors specified，10/18 production plugins implemented，M4-C software-verified，M4-D in progress**。Reference M4 完成至少需要：
