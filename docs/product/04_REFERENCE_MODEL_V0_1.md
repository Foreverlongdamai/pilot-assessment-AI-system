...
| Anchor 实现状态 | 18/18 已设计，5/18 production plugins 已实现；原 M4 实施计划已被取代，replacement plan 与 Task 3/7/8 amendments、D-028–D-030 已批准；Task 0–18 已完成，O1/O2/O3/O4/O5 capability 均为 `available`，`movement-events-v1` provider 为 `available`，下一步为 Task 19 O6 |
...
当前只可声称 **18/18 anchors specified，5/18 production plugins implemented**。Reference M4 完成至少需要：
