...
- 文档版本：1.0
- 目标前端：Windows WinUI
- 对应后端模型：revisioned Bayesian-network model store
- 核心要求：用户可以在前端画布中拖拽新增、删除、移动节点和边；所有结构和 CPT 修改必须一一映射到后端实际 BN。

本设计明确取消“v0 只读拓扑”边界。v0 前端允许编辑拓扑，但任何可运行或可发布模型仍必须通过后端验证。前端不是模型事实来源；后端保存的 canonical draft 及其 graph_version 才是模型事实来源。
...
- 显示 pending、invalid、needs_review、published 等状态；
...
- 默认不允许发布违反 pilot-assessment profile 的模型。
...
- 已提供的概率必须合法；
- 只有 compile 成功的 draft 才能成为 draft_runnable；incomplete/invalid draft 仍可保存，但不能 preview 或 publish。

若 Advanced draft 删除 TCP、PC、SM、OC 或改变输出契约，它不能作为 pilot-assessment v0.1 profile 发布。用户可以选择恢复 profile 要求，或以新的 major model profile 发布。该限制不阻止自由编辑，只防止把改变了语义的模型错误标成原 v0.1。
...
7. 执行 semantic completeness validation，并把 candidate 标为 draft_incomplete、draft_invalid、draft_runnable 或 draft_publishable；
8. 仅当 candidate 至少 runnable 时编译并运行最小 inference smoke test；失败时降为 draft_invalid 并保存诊断；
...
缺少 binding、CPT 或 required profile path 属于 semantic incomplete，可以保存到 draft；它阻止 runnable/publishable 状态，但不应让用户无法分步设计。Publish 始终要求 full validation、compile 和 smoke tests 全部通过。
...
- plugin：绑定已安装且受信任的 AnchorPlugin；
- anchor_clone：复制现有 anchor 的参数/binding 配置，生成新的 stable IDs，不复制或修改插件代码；
...
即把旧 CPT 的每一行复制到 X 的每个状态。这样新 edge 在初始时对推理零影响，保留修改前行为。Edge 和 CPT 会标记 needs_parameter_review。
...
- 保留其他参数；
- 物化完整 CPT；
- 标记 needs_parameter_review。

Add-edge 向导可以允许用户直接指定非零 weight、weakest-link 和 sigma。提交前必须预览：
...
- 新 CPT；
- KL divergence 或最大行差；
- 受影响 posterior 的 smoke-test diff；
- warning level。
...
删除 node 必须先执行 cascade preview。Preview 至少列出：
...
detach 删除目标 node 与 incident edges，但保留其他 nodes，并按预览策略迁移受影响 child CPT。cascade 只删除 preview 中明确列出的附加 nodes；不得把“可达后代”隐式全部删除。cancel 是前端动作，不发送 mutation。

preview_token 绑定 draft_id、graph_version 和 preview 内容。graph_version 改变后旧 token 失效，必须重新 preview。
...
State migration 必须先 preview，显示 M、R、q、迁移前后 CPT、归一化误差、monotonicity 和 inference diff。
...
允许保存 incomplete 或 invalid draft，便于分步设计；只有 runnable draft 可以执行明确标记为 non-formal 的 run.preview，只有 publishable draft 可以发布。正式 run.start 只接受 published revision_id。

### 7.2 Published revision

Published revision 不可变。发布操作：

1. 对 expected_graph_version 做并发检查；
2. 运行 full validation；
3. 编译 inference artifact；
4. 运行固定 golden smoke cases；
5. 生成 canonical hash；
6. 冻结 graph、CPT、anchor links 和 schema；
7. 原子更新 active published pointer。

发布后的任何编辑都自动从该 revision 创建新 draft，不得在原 revision 上原地修改。

### 7.3 Profile

pilot-assessment v0.1 profile 的 publish validation 要求：

- TCP、PC、SM、OC 四个输出 competency 存在；
- 输出 state schema 合法；
- 每个 competency 有至少一条有效 evidence path；
- 所有 active nodes 已配置；
- 33-node 默认模型的 required IDs 未被误用或重复。

Advanced 模式可以创建不同 profile，但必须修改 model_profile_id 和 major version。
...
下列操作必须 preview、确认、等待后端成功后再改变 committed graph：
...
编辑 draft 不会修改正在运行的 compiled model。发布时只需要短时间 exclusive publish lock：

1. 编译新 revision；
2. 原子切换 active pointer；
3. 已开始的 run 继续使用旧 snapshot；
4. 新 run 使用新 published revision。

若用户对 runnable draft 执行 run.preview，后端先为该 exact draft_id + graph_version 生成 immutable preview snapshot。随后发生的新编辑不会影响该次 preview；输出必须标记 non_formal、不得写入正式 AssessmentResult 或正式 result export。
...
### 11.2 Publish validation

在硬验证之外增加：

- model profile required nodes；
- 每个输出 competency 的 evidence reachability；
- 无 unconfigured active node；
- 无 unresolved needs_parameter_review，或有明确 reviewer waiver；
- 每个 active anchor 的 phase/event/window 聚合规则完整且唯一；
- generated/ordered CPT 通过 monotonicity；manual non-monotonic CPT 必须有 monotonicity_waiver；
- anchor link 与 modality schema 可解析；
- fixed inference smoke tests 通过；
- model hash 和 audit chain 一致。

### 11.3 Warning

以下问题可在 draft 中保存，但不能被忽略：
...
- model.draft.create / model.draft.get / model.draft.discard / model.draft.publish；
- model.draft.smoke_test；
- graph.snapshot.get；
...
- plugin.list / plugin.install（受信管理员流程）；
...
~~~json
{
  ...
  "params": {
    ...
    "actor": "user@example",
    "reason": "Add an expert-defined visual scanning sub-skill",
...
graph.operations.preview 必须返回 preview_token。正式 graph.operations.apply 携带该 token 后，后端再次确认：
...
失败事务只记录 audit event，不改变 graph_version。成功 undo/redo 产生新的 graph_version；只有 publish 才产生新的不可变 model revision。
