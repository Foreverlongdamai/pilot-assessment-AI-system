...
| 基线日期 | 2026-07-17 |
| 产品阶段 | M1/M2/M3、M4R、M5、M6 与 M7A Current Model Runtime 已工程验证；M7B Task 1–10 已完成 WinUI scaffold、强类型合同层、受监督 sidecar、受管 project/session、canonical task-scheme、global active/dim 模型图、图上节点/任务激活编辑、多独立节点浮窗，以及 schema/operator 驱动的 Raw Input/Evidence 编辑器；Task 11 起继续实现 BN state/parent/CPT 与后续 autosave、双语和运行结果工作区；M8 packaging 尚未设计；starter/synthetic `formal_run_authorized=false` |
