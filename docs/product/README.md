...
| 基线日期 | 2026-07-17 |
| 产品阶段 | M1/M2/M3、M4R、M5、M6 与 M7A Current Model Runtime 已工程验证；M7B Task 1–4 已完成 WinUI scaffold、强类型合同层、受监督 sidecar 通信与真实可见应用 shell，其余交互仍在实施；M8 packaging 尚未设计；starter/synthetic `formal_run_authorized=false` |
...
2026-07-17 用户确认 D-047–D-053：M7 改用完整独立节点、全局节点库、任务激活集合、默认只复制节点且复用 fixed parents、启用 parent closure、停用 parent 前级联确认、多浮动节点窗口、双语模型元数据，以及“autosave current scheme + automatic immutable RunSnapshot”。M7A 现已实现 global current nodes、TaskScheme activation/copy/cascade、atomic edge/state/CPT edits、autosave history/undo/redo、current preflight/run snapshot、legacy replay 和完整 `model.*` sidecar surface。fresh gate 为 current focused `42 passed`、compatibility `151 passed`、full repository `1684 passed, 3 skipped`；51 类 schema 零漂移，Ruff/format/ty/build 与仓库外 wheel smoke 通过。M7B Task 1–4 已建立正式 WinUI 工程，完成 M7A canonical DTO 的 C# 强类型映射和 Python fixture 往返验证，实现 4 MiB JSONL、并发 JSON-RPC client、隐藏 sidecar 进程监管、hello/capability/health/shutdown，并组合出带 Mica、NavigationView、状态栏、诊断页、偏好持久化和确定性退出的真实可见应用 shell。Task 4 fresh gate 为 shell state `4/4`、完整桌面 Unit `25/25` / Contract `2/2`、x64 Debug build `0 warning / 0 error`；真实窗口显示 backend `Ready`，关闭后应用和 sidecar 子进程全部退出。project/session、图编辑器、多浮窗、完整双语和运行结果界面仍待后续 Task 实现。M8 最终打包与科学验证仍未完成，starter/synthetic `formal_run_authorized=false`。完整状态见 [11_IMPLEMENTATION_STATUS.md](11_IMPLEMENTATION_STATUS.md)。
