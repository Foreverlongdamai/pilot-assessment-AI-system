...
| 基线日期 | 2026-07-13 |
| 产品阶段 | M1/M2/M3 后端里程碑已工程验证；M4 replacement Task 0–3 已分别由 `bc544bf`、`f56365c`、`928e9a4`、`e054620` 完成，方案 A/D-028 已批准，下一步为 Task 4；18/18 anchor 已设计，0/18 production plugins 已实现；M4 尚未工程验证 |
...
| 23 | [M4 replacement 实施计划](plans/2026-07-13-m4-anchor-evidence-availability-replacement-implementation-plan.md) | 开发、算法、审查者 | 已于 2026-07-13 获用户批准并按方案 A 修订；Task 0–3 已完成，下一步为 Task 4 |
...
M4 正式书面规格、[轻量工作流验证修订](specs/2026-07-13-m4-lightweight-workflow-validation-amendment.md)、[Task 3 Reference Candidate Binding 修订](specs/2026-07-13-m4-task3-reference-candidate-binding-amendment.md) 与 [replacement plan](plans/2026-07-13-m4-anchor-evidence-availability-replacement-implementation-plan.md) 均已批准，D-026–D-028 已接受。Replacement Task 0 已由 `bc544bf` 完成：旧 provisional heavy fixture 未进入提交历史；新的 10 秒 input-only fixture 以 452 PNG、468 个 manifest declared-path references 和 9,331 行 physical source tables 通过公开 M1→M2→M3 gate。Task 1 已由 `f56365c` 完成并锁定数值/JCS runtime；Task 2 已由 `928e9a4` 完成 breaking `AnchorResultV2` typed contract。Task 3 已由 `e054620` 完成 serializable semantic/reference contracts、session-bound runtime candidate 与三参数 exact binder；focused `35 passed`、全仓 `789 passed, 2 skipped`，Ruff/ty 通过，内部规格复核 P0/P1 均为 0。`src/pilot_assessment/anchors/` 现在只含 runtime reference binding 基础，不含任何 AnchorPlugin，因此真实状态仍是 18/18 anchor 已设计、0/18 production plugins 已实现，M4 尚未 engineering verified。AnchorResult v0.2 的 JSON Schema/export、AnchorPlugin registry/DAG、anchor-specific grids、evidence scorer、artifact/report 和 fingerprints 尚未进入生产代码；受管理存储 importer、BN、runner、sidecar 和 WinUI 同样尚未实现。下一步为 replacement Task 4。完整状态见 [11_IMPLEMENTATION_STATUS.md](11_IMPLEMENTATION_STATUS.md)。
