...
4. 查看、修改和发布贝叶斯网络及 anchor 参数；
5. 选择不可变 model revision 运行评估；
6. 从四项 competency 一直追溯到 sub-skill、evidence、原始时间窗和参数版本；
7. 导出可复现结果或诊断支持包。

界面不得掩盖缺失模态、低质量 evidence、未校准参数或模型科学验证状态。

## 2. 前端与后端责任

| 前端负责 | 后端负责 |
|---|---|
| 可视化、交互、表格编辑、输入预检查提示 | session schema、图、CPT、参数和 revision 的权威状态 |
| pending 操作、乐观 UI、冲突提示 | 原子 transaction、并发版本、验证与持久化 |
| 展示 canonical graph 和差异 | CPT 生成/迁移、DAG 编译、BN 推理 |
| 收集修改理由和用户确认 | audit event、provenance、content hash |
...
顶部状态条持续显示当前 project、session、published/draft model、未保存或未发布状态以及 backend health。任何结果页面都必须可见 model revision 和 run ID。
...
- 默认 model revision 及其 scientific validation badge；
- 项目数据位置和可用磁盘空间。
...
每张模态卡显示 present、export_pending、missing、invalid 或 not_applicable；同时显示时间范围、采样率、单位、checksum、同步质量和修复建议。导入前可展开 schema，看到后端接受的数据类型、必填字段、可选字段和示例。
...
- 左侧 palette：Competency、Sub-skill、Evidence/Anchor、Context；
- 中央无限画布：节点、连接 handle、缩放、框选、搜索、自动布局；
- 右侧 Node/Edge Inspector；
- 底部 Validation Console、operation history 和 pending 状态；
- 顶部 Guided Mode / Advanced DAG Mode、draft、validate、publish、undo/redo。
...
- 查看 shared evidence 的多个 parents；
- 打开 CPT、anchor parameter 或 safe formula 编辑器；
- 预览任何破坏性操作和 CPT migration；
- revision diff、回滚到父版本并从该版本创建新 draft。

详细事务和迁移规则以 [06_VISUAL_GRAPH_EDITOR_DESIGN.md](06_VISUAL_GRAPH_EDITOR_DESIGN.md) 为准。

### 4.5 Node Inspector

Evidence/Anchor node 显示：

- stable node ID、anchor ID、插件与算法版本；
- 输入模态、phase/event applicability 与 upstream dependencies；
- 公式说明、parameter schema、阈值和 quality gates；
- 当前 session 的 raw metrics、continuous score、D/A/U evidence、quality；
- source windows、diagnostics、missing policy 和 provenance；
- 绑定方式：existing plugin、clone、session field、safe composite formula。
...
- published revision 列表、parent、作者、时间、理由、hash；
- draft 与 base revision；
- topology、state、CPT、anchor parameters、binding 和 metadata diff；
- validation 与 inference smoke-test 结果；
- 从任意 published revision 创建新 draft；
- 发布确认和不可变性提示。

不能直接“修改已发布版本”。所谓 rollback 是从旧 revision 建立新 draft，验证后发布新的 revision。

### 4.7 Assessment Run

用户选择 session 与 published model revision 后先执行 preflight：

- 模态状态与同步质量；
- anchor applicability 和预计 coverage；
- reference path、event、baseline 等任务必需信息；
- model compilation、插件和 engine compatibility；
- 会阻断运行的 error 与只影响覆盖率的 warning。

run 页面显示阶段进度、已完成 anchor、取消按钮、日志摘要和 trace ID。run.start 只接受 published revision。Runnable draft 只能通过 run.preview 研究预览，结果必须显著标记 non_formal/non-published，不能进入正式结果或正式导出。

### 4.8 Results

第一层显示 TCP、PC、SM、OC posterior 分布和各自 coverage，不把 posterior 简化成无置信度的单一总分。
...
5. anchor continuous value、quality 和 phase/event breakdown；
6. 原始 source window、session artifact 和参数版本。

必须单独展示：

- missing/export_pending/invalid/not_applicable evidence；
- coverage 不足提示；
- model revision、bundle hash、run ID、算法版本；
- scientific validation status；
- “此结果用于研究/训练支持，不是认证决定”的产品声明。

### 4.9 Diagnostics 与支持包
...
放下节点后前端生成 transaction/op ID 和新的 UUIDv4 node_id 并显示 pending。后端验证该 ID，成功时在 canonical response 中确认同一 node_id 和 defaults；失败则移除 pending node。Semantic commit 成功后再以独立 expected_layout_version 提交 drop position；位置冲突不回滚已创建 node，只刷新并重试 layout。Evidence 节点必须完成 binding；未绑定时可留在 draft，但 publish 被阻断。
...
4. 可新增、删除、移动 node 和 edge，并与后端 canonical graph 一一对应；
5. rejected operation 能视觉回滚且不污染 draft；
6. CPT migration preview、validation、undo/redo、publish 完整可用；
7. 正式 run.start 始终锁定 published revision 并显示 provenance；non-formal run.preview 锁定 exact draft_id + graph_version；
8. 结果可从 competency 追溯到 source window；
9. 缺失证据与低 coverage 不显示成正常高置信度评分；
