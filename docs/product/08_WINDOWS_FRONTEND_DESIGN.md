...
- stable node ID、anchor ID、插件与算法版本，以及 `available/plugin_unavailable/not_implemented` capability；
- 输入模态、phase/event applicability 与 upstream dependencies；
- 公式前提、parameter schema、阈值和 scorer 版本；
...
Draft 新增 anchor 但插件尚不可用时，Node Inspector 显示 `plugin_unavailable` 并阻止 plan compilation/run；它是模型 capability 问题，不能显示成当前 session `not_computable`。Blocked report 中的 `not_attempted` inventory item 也不得伪装为 AnchorResult。
