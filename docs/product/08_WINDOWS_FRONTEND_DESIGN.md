...
| 字段 | 值 |
|---|---|
| 设计版本 | v0.2 expert-designer UX baseline |
| 平台 | Windows 10/11 |
| 推荐技术 | C#、.NET、WinUI 3 |
| 后端连接 | 本地 JSON-RPC sidecar |
| 核心原则 | 专家自由编辑、前后端同一 canonical recipe、后端权威、离线优先 |

> **当前权威补充：** 前端必须提供相互连接的 Evidence Designer 与 BN Designer。Evidence Designer 根据 backend operator/parameter schema 自动生成画布、表单和帮助，不在 C# 中复制计算逻辑。所有编辑 autosave；“应用到后续评估”只做最小技术可运行校验。详见 [Expert-Editable Evidence and Assessment Model Design](./specs/2026-07-15-expert-editable-evidence-and-model-design.md)。
...
4. 查看、修改 EvidenceRecipe/operator graph、贝叶斯网络、Anchor 参数/公式/scorer 与 CPT；
...
| 提供可选修改说明、apply 操作和历史入口 | 自动 audit event、provenance、content identity 与 immutable revision |
...
顶部状态条持续显示当前 project、session、applied/autosaved draft model、draft technical state 以及 backend health。任何结果页面都必须可见 model revision 和 run ID。
...
这是专家设计 evidence 计算方法的核心页面。它直接编辑后端 canonical `EvidenceRecipe`，至少包含：
...
- 顶部 Anchor lifecycle、clone、disable/retire、undo/redo、版本 diff 与“应用到后续评估”。
...
- stable node ID、Anchor ID、recipe revision、operator dependencies，以及 `executable/incomplete/operator_unavailable` capability；
- 输入模态、phase/event applicability 与 upstream dependencies；
- 当前 recipe summary、输入/输出、公式前提、parameter schema、阈值和 scorer；
...
- 与 BN node 的 binding，以及“在 Evidence Designer 中打开”入口。
...
- applied revision 列表、parent、作者、时间、可选说明、content identity；
- draft 与 base revision；
- topology、state、CPT、anchor parameters、binding 和 metadata diff；
- technical validation 与 preview 摘要；
- 从任意 applied revision 创建新 draft；
- apply 结果和不可变性提示。

不能直接“修改已应用版本”。所谓 rollback 是从旧 revision 建立新 draft，通过技术校验后 apply 为新的 revision。

### 4.8 Assessment Run

用户选择 session 与 applied model revision 后先执行 run preflight：
...
run 页面显示阶段进度、已完成 Anchor、取消按钮、日志摘要和 trace ID。run.start 只接受 applied revision。Executable draft 可通过 preview 直接试算；preview 结果标记 draft/non-applied，不改变历史 run。
...
7. run.start 始终锁定 applied revision 并显示 provenance；run.preview 锁定 exact draft_id + graph_version；
