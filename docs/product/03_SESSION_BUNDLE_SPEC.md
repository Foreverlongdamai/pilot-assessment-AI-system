...
| `invalid` | 文件存在且 integrity 可验证，但内容 schema、类型、时钟或质量不合格 | 禁止使用；给出修复诊断 |
...
`invalid` 表示文件路径与 checksum 完整性仍可验证，但内容 schema、类型、时间或质量不可用；checksum mismatch 是 bundle-level fatal integrity error，不能降级成可继续使用的 invalid stream。Loader 对 `present` 和 `invalid` 文件都执行路径安全与 checksum 验证。
...
原始时间戳不可被覆盖。若源文件暂时只有 session time，也应同时保留该列并声明其 clock。M2 的 `IngestionReadinessReport` 只验证 source timestamp 与 `clock_sync` 声明，不授权运行。

M3 生成的 aligned view 必须增加映射后的 int64 `t_ns`，表示相对 session origin 的时间。若原始文件已经导出 `t_ns`，M3 仍应根据 source timestamp 和 mapping 重算并验证，而不能直接信任。只有 aligned view 满足本节后续同步要求。
...
`source=bundle` 时 `stream_id` 必填，必须解析到 modality=`task_reference` 的 descriptor。该 descriptor 可以使用五种 stream status：`present/invalid` 的非空路径全部位于 `references/`；`export_pending/missing/not_applicable` 按第 5.1 节矩阵保持 fileless，由 readiness 与 M3 明确报告 reference 不可用或不适用。`source=model_bundle` 时禁止 `stream_id`，由锁定 model bundle 根据 `reference_id` 解析；七个 core modality 清单和 per-modality coverage 都不包含 task_reference。
...
该报告不验证 aligned `t_ns`、anchor applicability 或 competency coverage，并且 `formal_run_authorized=false`。M3 synchronization 和锁定 model revision 后，`run.preflight` 的 `RunPreflightReport` 才增加每个 anchor 的 data readiness 与每个 competency 的 evidence coverage；只有通过该运行门，pipeline 才能创建 AssessmentRun。
