...
本规范定义的是项目内部和标准交换使用的 **canonical Session Bundle**。模拟器的原始导出可以只有 `streams/` 与 `annotations/`，不要求自带 `manifest.json`。产品按 D-060 只读检查该目录，在受管项目 staging 中生成 manifest、checksum 和 canonical annotation，再进入本文规定的校验与运行路径；原始目录本身不被修改。
...
`units` 字段必须存在，但允许为空 object。若原始文件与可信 adapter profile 都没有声明单位，系统按 D-061 保持单位未声明，不要求用户补填、不猜测、不做隐式换算；原始数值仍由已匹配的固定 adapter/Evidence 方法解释，并在 provenance 中记录 `undeclared-pass-through-v1`。这是一项可审计的接口事实，不表示单位语义或科学算法已经验证。
...
设备若同时提供自己的 fixation，必须保留 detector_id/version/parameters；reference v0.1 仍从 raw gaze 重算。未来改用设备 detector 时，专家从当前方案创建 draft，选择或创建对应的 EvidenceVersion，并发布新的 AssessmentSchemeVersion；旧方案不改变。普通 detector/参数修改不要求 per-edit golden。
...
`source=model_bundle` 时禁止 `stream_id`，M3 不在 exact `AssessmentSchemeVersion` 及其 component closure 尚未锁定时猜测或读取 reference artifact，而是返回 `deferred_model_bundle_resolution`；锁定 scheme 后由 Run Preflight 根据 `reference_id` 解析 hash 与 task compatibility。该状态不是 missing/invalid。七个 core modality 清单和 per-modality coverage 都不包含 task_reference。
...
- M3 报告 residual、gap、overlap 和 association metrics；这些指标进入 diagnostics/provenance，不在锁定 exact scheme/component versions 后变成 M4 availability 或 evidence quality gate；
...
锁定 exact `AssessmentSchemeVersion`、全部 pinned component IDs/hashes 并解析 model-bundle reference 后，`run.preflight` 的 `RunPreflightReport` 检查 frozen inputs、任务前提、compiled Evidence/BN plan、operator/schema/engine 和版本兼容性；只有通过该运行门，pipeline 才能创建 AssessmentRun。实际 per-anchor raw availability 由 M4 结果给出，model-weighted competency coverage 由 M5 计算。
...
- Diagnostics 默认只显示 schema、错误、版本和脱敏摘要；D-077 取消自动生成 support bundle，任何未来显式导出仍不得包含原始模态数据；
