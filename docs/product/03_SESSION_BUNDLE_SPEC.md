...
- M3 报告 residual、gap、overlap 和 association metrics；task/anchor-specific tolerance 在锁定 model revision 后的 availability/preflight gate 判断；
...
M3 在同一 M1/M2 snapshot 上输出独立的 `SynchronizationReport`：包含七个 core modality 的完整 inventory、单独的 task-reference/annotation 结果、session window、clock/coverage/gap/duplicate/越界指标、policy/catalog/alignment fingerprints 与 `ready|ready_partial|blocked` disposition。M3 不插值或重采样，报告中的 `interpolated_rows=0`；该报告只决定能否进入 anchor availability，`formal_run_authorized=false`。

锁定 model revision 并解析 model-bundle reference 后，`run.preflight` 的 `RunPreflightReport` 才增加每个 anchor 的 data readiness 与每个 competency 的 evidence coverage；只有通过该运行门，pipeline 才能创建 AssessmentRun。
