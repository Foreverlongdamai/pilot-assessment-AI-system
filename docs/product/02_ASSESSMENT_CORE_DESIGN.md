...
**文档状态：** 产品 v0.2 后端总体设计；Evidence 计算部分已由 2026-07-15 expert-editable 规格重基线
**日期：** 2026-07-15
**上位文档：** [产品总览](./01_PRODUCT_OVERVIEW.md)

> **当前权威补充：** Evidence 计算以 canonical `EvidenceRecipe`、typed operator graph 和 generic compiler/executor 为唯一新扩展路线。下文旧 `AnchorPlugin` 章节只描述 Task 0–28 已实现代码及历史重放边界；普通参数、公式、图结构、scorer 或新 Anchor 不再要求 whole-Anchor Python plugin。详见 [Expert-Editable Evidence and Assessment Model Design](./specs/2026-07-15-expert-editable-evidence-and-model-design.md)。
...
- 允许专家从前端编辑 Evidence/BN 两张图、参数、公式、scorer 和 CPT；
...
- `ModelBundleManifest`：bundle 版本、recipe/Anchor catalog、operator requirements、兼容性、draft/lifecycle/verification/scientific/permitted-use 独立状态字段和 content identity。
- `GraphDefinition`：节点、边、层级、状态空间和显示元数据。
- `CptDefinition`：节点、父节点顺序、父状态组合和概率行。
- `ModelRevision`：不可变 applied revision、parent、diff、自动审计信息和 technical validation report。
- `InferenceRequest`：run snapshot、每 Anchor 单一聚合 evidence、phase/event breakdown metadata 和固定 applied revision。
...
7. 锁定 applied model revision，解析 model-bundle reference，并把 AnchorBinding、EvidenceRecipes、operator versions、参数、typed dependencies 与 BN 编译为 executable plan；availability precheck 只检查输入存在、适用性、配置、依赖和 capability，不按表现值或所谓质量判定。
...
- EvidenceRecipe identity 与 operator implementation versions；legacy replay 时另记 AnchorPlugin version；
...
- task profile IDs；
- operator/extension 与 BN engine compatibility；legacy bundle 可另列 whole-Anchor plugin compatibility；
- draft_validation_state（只在 draft manifest 中）或 revision_lifecycle（只在 applied manifest 中）；
- software_verification_status、scientific_validation_status 与 permitted_use，三个字段相互独立；
- author、created_at 和可选 change note；
...
- 用户选定的 output nodes 仍满足该 model 声明的可执行性约束；TCP/PC/SM/OC 只对 starter snapshot 默认存在。

拓扑修改只作用于 autosaved draft 并增加 graph_version；通过 apply 后才生成新 revision，且永不修改运行中的 revision。

### 7.3 CPT 与 EvidenceRecipe 参数
...
Recipe/operator 参数更新必须通过 `OperatorDefinition`/`EvidenceRecipe` JSON Schema，并重新计算受影响节点的依赖范围。阈值、公式和 scorer 变更自动进入 revision diff。
...
5. 原子写入 draft snapshot、diff、inverse patch 和自动 audit event。
6. graph_version 加一；不产生 applied revision。

只有 apply 才创建不可变 revision：后端再次检查 expected_graph_version，执行最小 technical validation 并编译 recipe/BN plan，生成 content identity，将 candidate 原子写入新的 `models/<revision-id>/`，再更新 project head。Apply 不运行 per-model pytest/golden，也不要求人工审批。

Revision 至少包含 parent ID、author/role、timestamp、可选 note、structured diff、technical validation report 和 content identity。回滚从历史 revision 创建新 draft，apply 后形成新 revision；不得删除或改写历史内容。
...
- Draft write 使用 `expected_graph_version` 的乐观并发控制；创建 draft 和 apply 同时校验 base revision_id。
...
Draft 修改命令返回新的 `graph_version` 或 structured validation error；只有 apply 返回新的不可变 `revision_id`。大型数据只通过受验证的本地路径引用。
...
- catalog 扩展、clone/disable/retire、参数修改、operator extension 与旧 revision deterministic replay tests；
...
- 使用一个紧凑多模态 fixture 验证 M1→M4R result/provenance 路径；不为每个专家 recipe 创建独立 golden，也不生成重型 fixture。
