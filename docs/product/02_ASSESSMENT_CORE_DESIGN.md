...
- recipe/operator DAG 与 fingerprint/cache-key material tests；M6 已覆盖 per-run source cache 与 preflight synchronization cache，跨 run 的通用性能 cache lifecycle/hit/invalidation 仍是可选优化而非正确性边界；
