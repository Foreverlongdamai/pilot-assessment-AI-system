...
- 18/18 computed Desired 与 18/18 computed Unacceptable 两套紧凑 aligned-input real-plugin workflow；后者 raw availability 必须为 100%，且 M4 不得生成 `invalid_quality`；
...
- 按已接受的 [M4 轻量工作流验证修订](./specs/2026-07-13-m4-lightweight-workflow-validation-amendment.md)，使用唯一 10 秒全模态 physical bundle 执行 M1→M4 result/provenance smoke；per-anchor 精确 golden、mixed/state 与 extension 分别由轻量测试承担，不生成四套 90 秒 bundle。
