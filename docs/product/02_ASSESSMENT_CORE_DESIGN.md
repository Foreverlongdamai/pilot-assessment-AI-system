...
4. 返回 `IngestionReadinessReport`；它只决定能否进入 synchronization，`formal_run_authorized=false`。
5. `synchronization` 验证 clock mapping、单调性、gap、offset、drift 和 residual，并生成 aligned `t_ns`。
6. 验证 task profile 所需的 phase、event、reference path 和 baseline。
7. 锁定 model revision，根据 binding 计算每个 anchor 和 competency 的数据可用性。
8. `run.preflight` 返回独立的 `RunPreflightReport`；fatal error 阻止运行，partial coverage 明确提示但可按模型规则继续。
