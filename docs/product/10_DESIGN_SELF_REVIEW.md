...
| 软件状态 | M1/M2/M3/M4R/M5 已工程验证；15 个 legacy/reference plugins、共享 primitives 与三个 providers 保留；M6 persistence/sidecar、M7 WinUI 尚未实现，`formal_run_authorized=false`，见 [11_IMPLEMENTATION_STATUS.md](11_IMPLEMENTATION_STATUS.md) |
...
- 尚未实现：durable project/session/model/run persistence、managed artifact lifecycle、JSON-RPC sidecar、正式 run orchestration 和 WinUI；这些分别属于 M6–M7。
- 已停止：旧 replacement Task 29–36 与 fixed exact-18 completion gate。
- 方向重基线本身不是科学验证；M4R 的工程完成证据以实施计划和 fresh commands 为准。

### 8.5 后续门槛

M4R 与 M5 implementation plan 均已完成。下一步先形成 M6 persistence/runtime protocol 正式规格，再实现 durable repository、managed artifacts、JSON-RPC sidecar 与 run lifecycle；不直接跳到 M7 WinUI。旧 fixed-plugin 计划不得被继续执行或机械改名复用。
...
- 完成结论只适用于 M5 transport-neutral/in-memory backend。SQLite/managed artifact/JSON-RPC/run lifecycle 属于 M6，WinUI 属于 M7，科学有效性仍待专家研究；`formal_run_authorized=false` 保持不变。
