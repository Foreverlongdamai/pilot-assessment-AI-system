...
| 结论 | M1–M3 可作为已验证交接基线；M4 书面设计与轻量验证修订已获批准，replacement Task 0–28、M4-A/M4-B framework、M4-C/M4-D/M4-E stage gates 与 O1–O12/H1–H3 已完成，但 M4 整体仍未工程验证 |
| 软件状态 | 2026-07-10 审查时尚无实现；截至 2026-07-15，M1/M2/M3 已工程验证，M4 Task 0–28 已完成，M4-C/M4-D/M4-E 已关闭，O1–O12/H1–H3 capability 与三个 preprocessing providers 均为 `available`，共享 primitives 已实现，18/18 specified、15/18 production plugins 已实现，`formal_run_authorized=false`，完整 Core alpha 与 Gate B 仍未完成，见 [11_IMPLEMENTATION_STATUS.md](11_IMPLEMENTATION_STATUS.md) |
| 科学状态 | 参考评估模型为 engineering_default；synthetic fixture 为 not_supported |
| M4 修订 | 2026-07-13 完成 anchor-computation、lightweight-workflow、Task 3、Task 7 与 Task 8 amendments 并获明确或授权默认批准；D-026–D-030 与 replacement plan 已接受；截至 2026-07-15 replacement Task 0–28 已完成，M4-C/M4-D/M4-E 已关闭，下一步为 Task 29 H4；18/18 specified、15/18 production plugins 已实现，M4 整体尚未 engineering verified，无科学有效性声明 |

## 1. 结论边界

本文固定记录 2026-07-10 的**产品与计算设计闭环**审查，表格中的审查日期不随实现推进而改写；当前实现状态仅作后续注记。本文不是软件实现验收，也不是航空评估科学有效性证明。后续实现证据以 [11_IMPLEMENTATION_STATUS.md](11_IMPLEMENTATION_STATUS.md) 为准。2026-07-13 M4 amendment 见第 7 节；它重新打开并关闭了旧 reference-v0.1 quality-gate 语义冲突，对 M4 的状态、计算和验收规则具有优先级，但规格获批本身不表示生产代码已经实现。Replacement Task 0–28 的逐任务实现证据以 [replacement plan](plans/2026-07-13-m4-anchor-evidence-availability-replacement-implementation-plan.md)、review ledger 与对应 Git commits 为准。
...
| M4-SR-11 | 分段确认曾被误写成完整书面批准 | 候选阶段明确区分两者；用户于 2026-07-13 依次批准完整 M4 规格、轻量验证、replacement plan 与 Task 3 amendment，并明确授权休息期间中间方案默认批准；Task 7/8 amendments 在两路 P0/P1 终审 PASS 后按该授权接受，D-021～D-030 均有受控迁移记录；截至 2026-07-15 Task 0–28 已完成，M4-C/M4-D/M4-E 已关闭，下一步为 Task 29 H4 |
