...
- [x] 写入 D-066–D-071：software-copy system owner、无 project Model Studio、project/run boundary、legacy import、single-writer、loaded-source identity 边界。
- [x] 将 M8B 设计和自审状态从 review candidate 改为已批准。
- [x] 更新路线图：M8B-0 先修正 ownership，M8B-1/2 再实现 source provenance/operator 扩展闭环。
- [x] 明确 M8B-0 未完成前，M8A ZIP 仍是旧 project-scoped engineering build。
...
- [x] 定义 `SystemDescriptor`，只保存相对 `database_path`、`model_library_id`、format/product/seed identity 和创建时间。
- [x] 实现 `SystemStore.create/open/open_or_create()`，固定目录为 `system.json`、`model-library.sqlite3`、`staging/model-edit/`。
- [x] 复用 SQLite kernel/migrations，但 system service 不暴露 project/session/run 表。
- [x] 校验目录可写、locator 不含绝对路径、descriptor/database metadata 一致。
- [x] 用独占 lock file 保证同一软件副本只有一个 system writer；正常关闭释放，陈旧锁按进程身份安全恢复。
...
- [x] 发布 `ModelGraphSnapshot` v0.2：使用 `model_library_id`，不再把 project ID 写入 current graph identity。
- [x] 发布 `ModelEditSessionStatus` v0.2：使用 `model_library_id`。
- [x] `CurrentModelWorkspaceService`、edit session 和 semantic hash 全部使用 model-library scope。
- [x] 保留旧 v0.1 RunSnapshot/历史 JSON 的兼容读取，不做字符串字段替换。
- [x] 同步 C# source-generated contract，普通 UI 不显示 model library ID。
...
- [x] sidecar 启动即定位并打开 `SystemApplication`，无 project 也完成 starter seed。
- [x] `SystemApplication` 独占 current model、model edit session、component/scheme compatibility services、operator/source registries与 system audit/idempotency。
- [x] 所有 `model.*`、`operator.*` 与兼容 model-library RPC 路由到 system app。
- [x] `runtime.status` 分别返回 `system_ready` 与 `project_open`；关闭 project 不关闭 system app。
- [x] model mutation 的 transaction/audit/checkpoint 全部落 system database/edit workspace。
...
- [x] `ProjectApplication.create/open` 注入已打开的 `SystemApplication`。
- [x] 删除新 project 的 starter/current-model seed；project model tables 保留为空的兼容结构。
- [x] project 继续拥有 immutable execution component materialization，但输入来自 system 的 exact clean closure。
- [x] preflight 同时记录 SessionRevision lock 与 system model lock；run start 前重新检查 stale。
- [x] `RunRepository.create_current()` 只验证已冻结 snapshot/preflight，不查询 project-local current nodes/schemes。
- [x] pipeline 只使用 project-owned frozen materialization，run 开始后不读取 mutable system store。
...
- [x] portable mode 将软件根目录显式传给 sidecar；development mode 使用 repo-local ignored system root。
- [x] backend 生成 stable project ID；项目页只要求可读名称和空目录。
- [x] sidecar Ready 后立即加载 TaskScheme/Model Studio，不等待 project activation。
- [x] project close/switch 只清理 Session/Run/Result context，不清理模型列表或 dirty model edit session。
- [x] 项目页说明改为“项目保存 Session、运行、结果和 artifacts；系统模型由当前软件副本共享”。
...
- [x] 为 legacy fingerprint/import receipt 增加 system-owned migration records。
- [x] 相同 ID/hash 复用；缺失 ID 原样导入；同 ID/不同内容生成确定性 imported ID。
- [x] 只改写 typed node/scheme/parent/CPT/layout references；无法验证则整笔回滚。
- [x] 导入成功后 legacy tables 保持原样只读；重复打开幂等 no-op。
- [x] 最多恢复一套 legacy dirty edit session；第二套冲突保留原 workspace 并返回 recoverable error。
...
- [x] 无 project：列出 starter scheme，修改一个微型 layout/参数并 Save all。
- [x] 创建 Project A 与 Project B：两者都看到同一修改，数据库均无 current-model rows。
- [x] 在 A 用微型 Session 创建 run snapshot；随后修改 system node。
- [x] A 的旧 snapshot/hash 不变；B 的新 preflight 使用新 identity。
- [x] 移动 A 整个目录后仍可打开和读取旧 run。
- [x] sidecar 第二 writer 被稳定拒绝。
...
- [x] 构建时确定性创建 starter `system/` 与 `system-model-baseline.json`。
- [x] release 扫描继续阻止用户 project/session/result，但允许系统 baseline 数据库。
- [x] 从仓库外启动产品：未打开 project 进入 Model Studio；创建两个项目；验证共享模型。
- [x] ZIP 不包含测试产生的 project/session/result，系统 baseline 保持 clean。
- [x] 记录版本、hash、命令、通过项与明确未完成项。
- [x] 只选择性提交 M8B-0 文件，不卷入当前工作树无关改动。
