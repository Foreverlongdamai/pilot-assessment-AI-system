...
- [ ] 先写合同测试，覆盖完整 recipe/definition round-trip、frozen nested parameters、非法 ID/NaN/extra field 拒绝，以及 incomplete graph 可以解析。
- [ ] 运行：

      uv run pytest tests/contracts/test_evidence_recipe.py -q

  预期：collection 因 evidence_recipe 模块不存在而失败。

GREEN：

- [ ] 实现枚举 RecipeLifecycle、RecipeScientificStatus、InputBindingKind、OperatorFamily、PortCardinality、TemporalSemantics、TraceCapability、OperatorImplementationSource、OutputRole 和 ScoringMode。
- [ ] 实现 PortType、OperatorPortDefinition、ParameterUiDefinition、OperatorDefinition。
- [ ] 实现 RecipeAnchor、RecipeInputBinding、NodePortReference、RecipeNode、RecipeEdge、RecipeGraph、RecipeOutputBinding、RecipeScoring、RecipeDocumentation、RecipeUiMetadata、EvidenceRecipe。
- [ ] 对所有 JSON mapping 使用 freeze_json_mapping；仅做结构约束，不在 DTO 层阻断 incomplete graph。
- [ ] 从 contracts/__init__.py 导出公共类型。
- [ ] 再运行 focused test，预期全部通过。
- [ ] 运行：

      uv run ruff check src/pilot_assessment/contracts/evidence_recipe.py tests/contracts/test_evidence_recipe.py
      uv run ty check src/pilot_assessment/contracts/evidence_recipe.py
