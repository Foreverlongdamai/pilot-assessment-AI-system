...
> 状态：Approved / implementation entry ready；production code 尚未开始
...
- [ ] 写 safe integer、surrogate、non-string key、NaN/Infinity、map-order independence、type/schema separation 和 deterministic hash tests。
- [ ] 写 legacy `typed_json_sha256`/known fingerprint regression，确保 refactor 前后 bytes/hash 不变。
- [ ] 运行：

      .venv\Scripts\python.exe -m pytest tests/model_library/test_identity.py tests/anchors/test_fingerprint.py -q

  预期 RED：`pilot_assessment.model_library.identity` 尚不存在；legacy tests 继续可收集。

GREEN：

- [ ] 实现 `jcs_bytes()`、`typed_content_sha256()` 与 frozen semantic projection helper。
- [ ] legacy functions 仅委托 generic primitives，不改变公开名称、type IDs 或输出。
- [ ] 复跑 focused tests。
