...
> **For agentic workers:** REQUIRED execution mode is **INLINE**. This file remains the M8 roadmap rather than a file-level task list. The user explicitly authorized M8 execution on 2026-07-20；M8A has completed through the approved [M8A implementation plan](2026-07-20-m8a-portable-windows-release-implementation-plan.md). Do not infer that M7 user acceptance or later M8B–M8E gates are complete.

| 字段 | 值 |
|---|---|
| 计划类型 | Pre-UAT implementation outline |
| 日期 | 2026-07-18 |
| 状态 | **路线图已批准；M8A 已完成，M8B–M8E 待实施** |
...
| 当前动作 | 以已稳定的 M8A release layout 为输入，编写并执行 M8B 正式计划 |
...
2. 防止 live Python source、文档、备份和交付在最后临时拼接；
...
- release layout 已稳定，足以供 M8B/M8D 引用。
...
> **2026-07-21 ownership correction：** 用户已批准 D-066–D-071。M8B 先执行 M8B-0，将 current ModelNode/TaskScheme/edit session 提升到每套软件副本的 `system/`，使 Model Studio 无 project 可用，并让 Project 只拥有 Session/Run/Result/artifact；再执行 M8B-1 source provenance 与 M8B-2 operator 扩展闭环。详见 [M8B 正式规格](../specs/2026-07-21-m8b-system-owned-model-library-and-editable-backend-provenance-design.md) 与 [M8B-0 实施计划](2026-07-21-m8b0-system-model-ownership-implementation-plan.md)。本段早期把 canonical model/edit session 默认为 project-owned 的措辞均由该规格取代。
...
- 12 类手册的中文和英文内容；
- 最终 M7/M8 UI 截图；
- editable Python source、backup、release 等 M8 功能说明；
...
| 7 | backup/migration/troubleshooting | 等 M8D 行为冻结后编写 |
...
## 7. M8D — Backup, Migration and Diagnostics

### 7.1 目标

在不把用户数据打进产品包的前提下，使每个用户自己的受管项目能够安全备份、恢复、移动、升级和生成脱敏支持信息。

### 7.2 候选范围

1. 版本化 `.paprojbackup` contract；
2. SQLite consistent backup/checkpoint；
3. managed files inventory 和 hashes；
4. staging、原子完成和中断清理；
5. restore inspect、path traversal、hash、空间和版本检查；
6. 恢复到新/空目录，不默认覆盖；
7. 数据库 schema migration 前的安全备份策略；
8. RunSnapshot source/runtime/operator identity、source snapshot artifact 和缺失依赖诊断；
9. WinUI Backup/Restore/Migrate/Diagnostics 入口；
10. 默认脱敏 support bundle。

### 7.3 关键不变量

- backup 不改变打开项目的 canonical state；
- restore 失败不留下半恢复项目；
...
- support bundle 默认不包含 raw modalities、pilot-camera、participant identity 或完整私有内容；
- 产品升级不自动删除或覆盖旧软件目录、旧 runtime 或专家修改过的 live source tree。

### 7.4 轻量验证

- 一个微型项目完成 backup → move → restore → reopen → result replay；
- 在备份写入中途故障后原项目不变、临时文件可清理；
- 篡改文件/hash 的备份被拒绝；
- path traversal entry 被拒绝；
- 已存在目标目录不会被静默覆盖；
- 缺失历史 source snapshot 或 runtime dependency 产生明确依赖诊断；
- support bundle 隐私扫描通过。

### 7.5 M8D 退出门

- 用户能从 UI 备份和恢复项目；
- 项目在另一目录/设备上打开并保留 nodes/schemes/sessions/runs/results；
- migration 前后有可恢复路径；
- diagnostics 对普通用户可理解，对维护者足够定位问题；
- 任何用户数据仍不进入产品 release ZIP。
...
8. 执行 project backup/restore/move vertical slice；
9. 检查所有 DOCX、交叉引用和版本；
10. 形成用户最终验收记录和交付清单。
...
- runtime/source import/backup/doc build 的失败都必须有明确阶段和恢复动作；
- 不用“质量差”过滤合法差表现数据。
...
- release 和 support bundle 都执行隐私/路径扫描；
- 默认离线、零监听端口；
...
- 先完成一条最小垂直切片，再扩展覆盖；
- 对 package/live-source identity/backup 等高风险不变量使用 contract-first 和 focused tests；
...
- M8A 已获单独授权并关闭 engineering gate，已生成产品 ZIP 和稳定的 live-source release layout；
- M8B–M8E 尚未完成，backup 格式、DOCX pipeline、clean tagged release candidate 与最终交付验收仍不存在；
- 下一开发动作是基于 M8A layout 推进 M8B；M7 用户验收及返修可并行，但必须在 M8E 前关闭。
