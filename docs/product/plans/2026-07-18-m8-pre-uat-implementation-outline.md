...
> **For agentic workers:** REQUIRED execution mode is **INLINE**. This file remains the M8 roadmap rather than a file-level task list. The user explicitly authorized M8 execution on 2026-07-20；M8A、M8B 与 M8C-0 已分别按正式计划完成。Do not infer that M7 user acceptance, M8C-1, M8D or M8E gates are complete.

| 字段 | 值 |
|---|---|
| 计划类型 | Pre-UAT implementation outline |
| 日期 | 2026-07-18 |
| 状态 | **路线图已批准；M8A、M8B、M8C-0 已完成，M8D 为下一阶段；M8C-1/M8E 待实施** |
...
| 当前动作 | 制定并实施 M8D backup、restore、migration 与 diagnostics 正式计划 |
...
> **2026-07-21 execution update：** M8C-0 已完成。12 类 catalog/schema、固定 Python/Node 工具链、DOCX template、Markdown parser、交叉引用、C4 assets、三份代表手册与 portable review-doc verifier 已通过；28 页代表手册已逐页检查。M8C-1 仍等待 M7 UAT、M8D 与 M8E 的最终行为和截图，不得提前关闭。
...
- M8C-1、M8D 与 M8E 尚未完成，完整双语文档套件、backup 格式、clean tagged release candidate 与最终交付验收仍不存在；
- 下一开发动作是制定并实施 M8D；M7 用户验收及返修可并行，但必须在 M8E 前关闭。
