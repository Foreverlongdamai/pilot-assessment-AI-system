...
3. 让用户直接针对完整候选反馈，并以新的 candidate sequence 收口必要返修；
...
flowchart LR
    ...
    B --> E
    D --> E
    E --> UAT["User acceptance\nof complete candidate"]
    UAT -->|accepted| F["Rebuild v0.1.0\nfrom accepted source"]
    UAT -->|changes| R["Repair and issue\nnext RC"]
```

默认顺序是：

1. M8A 已按用户单独授权先稳定便携 runtime、目录和启动链；
2. 已在稳定 runtime 上保证第一方 Python 源码直接可编辑、全局生效，并实现项目迁移；
3. 完成 D-055、M8C-1 候选手册/截图与 M8E release pipeline；
4. 从 clean tagged source 生成并自动隔离验证 `v0.1.0-rc.1`；
5. 用户直接验收完整候选；接受则从对应 source 重建正式版本，要求修改则产生下一候选。

## 3. 完整候选后的用户验收门

### 3.1 目标

由用户亲自验证完整候选是否符合真实操作预期。该验收不再阻止 M8C-1/M8E 构建，但继续阻止候选被称为 accepted final release。

### 3.2 输入

- 完整 `v0.1.0-rc.1` ZIP、checksum 与 candidate metadata；
- 一个用户可理解的受管项目；
- 一个轻量 canonical Session Bundle 与一个只含 `streams/`/`annotations/` 的 raw source；
- 候选内双语手册、release notes、known limitations 与自动隔离验证证据。
...
- 被接受或要求返修的用户操作路径和候选截图清单；
- 对 M8 候选设计的变更清单。

### 3.5 停止条件

候选构建和自动工程验证通过后保持 `user_acceptance=pending`。若用户认为核心操作方式需要改变，修复并形成新的 release candidate；不得覆盖或重命名旧候选。只有用户接受且内容未再变化时，才从接受的 source commit 重建正式 `v0.1.0`。
...
> **2026-07-21 execution update：** M8C-0 已完成。12 类 catalog/schema、固定 Python/Node 工具链、DOCX template、Markdown parser、交叉引用、C4 assets、三份代表手册与 portable review-doc verifier 已通过；28 页代表手册已逐页检查。D-077 将未发布 backup 手册迁移为 portability 手册，M8D 行为现已工程冻结。D-078/D-080 允许 M8C-1 现在生成隐私审核后的 `release-candidate` 手册截图；它们在用户接受前不得标为 final。
...
| 7 | system distribution/project portability/troubleshooting | M8D 行为已冻结；正文仍属于 M8C-1 |
...
> **2026-07-21 execution update：** M8D fresh engineering gate 已关闭。显式 current-system capture、动态 baseline、完整 project directory copy/reopen/replay、typed system/project Diagnostics 与 disposable package verifier 均通过；详细证据见 [M8D Verification](../reviews/2026-07-21-m8d-current-system-packaging-verification.md)。这不关闭 M7 用户验收、M8C-1 或 M8E。
...
5. 在仓库外 disposable extraction 中使用 restricted `PATH` 执行自动 portable verification，并如实记录未执行的 Sandbox/独立机器项；
...
- 一个候选 ZIP 通过已声明的自动隔离验证矩阵；
- 交付物可由另一位用户独立完成首次运行；
- 产品包无用户数据和开发机私有状态；
- release materials、DOCX、source identity 和第三方 notices 齐全；
- 候选 manifest 明确为 `user_acceptance=pending`；最终用户验收单独决定是否重建 accepted `v0.1.0`；
...
正式文件继续使用仓库现有的 `YYYY-MM-DD-milestone-topic-design.md` 和 `YYYY-MM-DD-milestone-topic-implementation-plan.md` 命名约定，并分别为 M8A、M8B、M8C、M8D、M8E 建立正式规格与实施计划。本文件不会被重命名为“Approved plan”；它继续作为路线演进记录。M8E 当前以已批准的 2026-07-21 正式规格和计划为执行权威。
