...
2. 把整个压缩包解压到较短且可写的本地路径，例如 `D:\PilotAssessment-0.1.0-rc.1`。
3. 保持 `PilotAssessment.Desktop.exe` 周围的全部子目录不变，不要只移动 EXE。
...
双击 `PilotAssessment.Desktop.exe`。前端会自动启动一个本地 Python sidecar 子进程；JSON-RPC 只通过 stdin/stdout 传输，不监听 TCP 端口。SQLite 是由 Python 进程直接读写的嵌入式文件数据库，不是需要单独启动的程序。
