...
scope = "说明如何构建、在仓库外验证并交付 Windows x64 v0.1.0-rc.2，同时不提前宣称最终用户验收。"
...
RC.2 只改变 portable 根目录、launcher 与不可见的 runtime locator，手册中展示的 WinUI 页面没有视觉变化，因此复用 RC.1 的十张截图 bytes。screenshot manifest 保留原始 capture source identity，并显式记录 `reused_from_release_label=v0.1.0-rc.1`；不得把它们伪写成重新捕获。
...
6. 创建 annotated `v0.1.0-rc.2` tag，并证明它 peel 到 `HEAD`。
...
- code、model、runtime、screenshots 或 executable behaviour 变化时创建新 candidate（如 `rc.2`），重新捕获相关 evidence 并执行 external verification；
