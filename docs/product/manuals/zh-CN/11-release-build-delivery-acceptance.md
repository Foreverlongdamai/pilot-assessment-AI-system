...
scope = "说明如何构建、在仓库外验证并交付 Windows x64 v0.1.0-rc.3，同时不提前宣称最终用户验收。"
...
RC.3 修复 Assessment 技术执行、任务栏图标、全局删除节点和按住拖动。Model Studio 是受影响页面，必须从最终 UI source 重新捕获中英文截图；其余视觉未变化的页面可以明确记录为从 RC.2 复用，不得伪写成重新捕获。
...
6. 创建 annotated `v0.1.0-rc.3` tag，并证明它 peel 到 `HEAD`。
